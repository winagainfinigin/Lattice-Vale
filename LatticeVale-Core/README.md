...
- Resource policy v12 is schema **12** and uses one canonical host-memory-budget API for generation, runtime-policy document validation, and state audit. Policy generation immediately self-validates before downstream configuration continues.
- Installer options schema is **22**, managed repair refresh revision is **4**, and canonical derived-state schemas are declared only in `compatibility.conf`. Recognized 14.x managed installs migrate through the existing preservation-first Resume / repair path.
...
- v14.6.0 migrates recognized historical/versionless managed options through current schema 22 without resetting user/application state. Managed repair refresh revision 4 converges canonical hardware/backend/resource state on repair while preserving durable choices and application data.
...
- Adaptive per-service CPU/RAM ceilings are optional and `compose.override.yaml` remains the final user override layer. Current policy v12 keeps the aggregate-budget model but moves host reserve, DirectML reserve, CPU quotas, service-memory water filling, model/context/concurrency, GPU coordination, and allocator/database tuning into the canonical architecture library. Calculations scale from live WSL CPU/RAM, enabled service topology, actual model inputs, and measured backend/GPU/UMA capability rather than fixed machine-size bands. DirectML runtime health is fingerprinted separately from resource topology so safe Ollama fallback does not make a valid policy stale; resource-relevant managed-Ollama acceleration changes can still trigger rebudgeting. Repair/start automatically migrates enabled policy-v2 through policy-v11 overlays and refreshes when canonical hardware/topology/options inputs change.
