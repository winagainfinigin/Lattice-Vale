# LatticeVale v14.6.1 — Technical README
...
- **Current schema/policy correction:** options schema **23** persists `useGpuAcceleration` and `directmlFallbackPolicy`; runtime policy **13** conserves one WSL CPU envelope across enabled services. DirectML reserves about 25% host CPU plus about 10% scheduler headroom, leaving about 65% aggregate Docker CPU; non-DirectML operation leaves about 90% aggregate Docker CPU. CPU-only is a supported durable selection. Ollama text fallback and Honcho embedding roles are independent, and DirectML fallback `none` remains fail closed.
- **v14.6.0 canonical architecture:** `latticevale_arch.py` is the shared schema/capability/fingerprint/resource-policy library. `hardware-capabilities.py`, `backend-capabilities.py`, `runtime-policy.py`, and `diagnostics.py` generate/verify atomic derived state under `data/latticevale/`; durable user intent remains in `install-options.json`.
- Hardware detection is split into a Windows installer snapshot plus live WSL topology. DirectML `/dev/dxg`, CUDA, ROCm, Vulkan/DRM, native-Windows Ollama, and CPU are classified independently; zero Linux-native GPU adapters is not treated as proof that DirectML is unavailable.
- Backend selection preserves explicit adapter/backend intent while deriving an effective safe backend from current capability/health. Runtime health is tied to the hardware fingerprint, and explicit missing adapters are never silently redirected to an unrelated Windows GPU.
- Resource policy v13 uses runtime-policy schema **13** and uses one canonical host-memory-budget API for generation, runtime-policy document validation, and state audit. Policy generation immediately self-validates before downstream configuration continues.
- Installer options schema is **23**, managed repair refresh revision is **4**, and canonical derived-state schemas are declared only in `compatibility.conf`. Recognized 14.x managed installs migrate through the existing preservation-first Resume / repair path.
- Read-only diagnostics are first class: existing-stack installer option 8 and `manage.sh diagnose*` expose Windows/WSL hardware, backend reason codes, adapter selection, health, and policy without requiring the stack to have reached final `.configured` state.
- **Same-version 14.6.0 GPU-memory/fallback hardening:** Windows adapter memory is correlated by stable/PNP identity across DXDiag and the display-driver registry; WMI/DWORD memory is only a lower bound. Canonical DirectML admission separates dedicated VRAM from bounded UMA/shared memory, and the gateway consumes that capacity when `torch_directml` cannot report memory. Transient DirectML failure changes active fallback/health but not resource-relevant backend identity, preventing a healthy Ollama fallback from making `runtime-policy.json` appear stale. Option 8 stages the current canonical validator + fresh Windows snapshot when auditing older stacks.

- **v14.5.47 Same-version Hotfix 1:** Resume / repair schema validation now accepts the current installer schema 21 and repair-origin schema 21; future schema 22+ still fails closed. The initial 14.5.47 package could write schema 21 and then have `configure-stack.sh` reject that same file. Rerun the revised full 14.5.47 release; no uninstall or data reset is required.
- **v14.5.47 Same-version Hotfix 2:** resource-policy generation and verification now share one canonical host-memory budget. This fixes Resume / repair rejecting a freshly generated DirectML policy (for example 11,962 MiB WSL RAM correctly yields a 2,990 MiB DirectML/host reserve and 8,972 MiB container budget). The verifier now checks `RESERVE_MIB`, `DIRECTML_HOST_RESERVE_MIB`, and `BUDGET_MIB`; DirectML does not depend on Linux-native GPU inventory, so `/dev/dxg` systems remain compatible even when managed Ollama resolves to CPU.
- v14.5.47 fixes the DirectML WSL selection layer by exporting the saved Windows adapter through `MESA_D3D12_DEFAULT_ADAPTER_NAME` before every `torch_directml` probe/worker import, while retaining exact in-runtime adapter matching.
- The selected adapter's Windows DXDiag dedicated VRAM is persisted as `directmlVramMiB` and used only as an admission fallback when `torch_directml.gpu_memory()` is unavailable or unusable. DirectML remains fail-closed when adapter identity or capacity cannot be established.
- DirectML hard-fallback markers now carry a runtime fingerprint; legacy markers and material WSL/GPU/runtime changes get one retry, while unchanged repeated failures remain safely on Ollama fallback. `directml-gateway.sh diagnose` and `tools/Audit-LatticeVale-Gpu.ps1` provide read-only diagnostics.
- Managed Ollama now supports `vulkan` in addition to Auto/CPU/NVIDIA/AMD-ROCm. Vulkan requires an existing WSL DRM render node, passes `/dev/dri` plus `OLLAMA_VULKAN=1`, uses conservative CPU-style resource admission until runtime proof, and accepts GPU execution only after `ollama ps` reports real offload.
- Windows/vendor display drivers remain external; NVIDIA CUDA, AMD ROCm, DirectML, native-Windows Ollama, and CPU-safe paths are retained.
- Windows Tailscale remains an explicit host integration, while LatticeVale no longer offers or manages Ubuntu Pro for WSL; external Ubuntu Pro state remains untouched. SearXNG + Valkey remain local stack services.
- v14.6.0 migrates recognized historical/versionless managed options through current schema 23 without resetting user/application state. Managed repair refresh revision 4 converges canonical hardware/backend/resource state on repair while preserving durable choices and application data.
- Current v14.6.1 hardens Windows Tailscale remote access: same-host Serve checks are no longer treated as remote proof; client/DNS/HTTPS/relay/listener/Matrix checks run first and a current-run second-device HTTPS challenge is required for remote `PASS`.
- Current v14.6.1 extends the deterministic release contract to 144 fixtures. The suite retains all v14.5.47 GPU recovery, v14.5.46 onboarding, v14.5.45 PowerShell, v14.5.44 DirectML preflight, v14.5.43 migration, and earlier preservation guarantees while adding adaptive architecture/release-policy gates and the v14.5.2/v14.6.0 eight-option continuity gate.

- v14.5.46 adds GPU-aware installer onboarding: Windows GPU inventory + WSL DirectML/Ollama prerequisite probes produce a hardware-specific recommendation, while selected installer-owned WSL prerequisites are reused when healthy or provisioned when missing. NVIDIA prefers managed Ollama CUDA when WSL `nvidia-smi` is verified; AMD ROCm is recommended only with real `/dev/kfd` + `/dev/dri`; otherwise healthy DirectX 12 AMD/Intel/Qualcomm systems are steered toward DirectML. Windows/vendor display drivers remain external.
- v14.5.45 removes PowerShell generic-collection construction through `New-Object` across installer, uninstall, repair/reset, and relay paths. Direct `.NET ::new()` constructors and explicit `.ToArray()` returns avoid the PowerShell 7.6 `Argument types do not match` binder failure that could occur immediately after a successful DirectML preflight. Users do not need manual source edits or a PowerShell downgrade.
- v14.5.44 gives DirectML a dedicated WSL preflight: direct `/dev/dxg` path test, normal-user→root retry, separate D3D12/DXCore bridge-library status, and an optional real tensor probe when the installer-owned DirectML venv already exists. It no longer reuses the Ollama GPU prerequisite parser for DirectML admission, preventing false `/dev/dxg` negatives on otherwise healthy WSL2 GPU projection. Probe failure, confirmed DXG absence, and incomplete bridge libraries are reported as distinct states.
- v14.5.43 makes **Resume / repair** a cumulative migration path for any older recognized installer-managed LatticeVale stack. It proves ownership before adoption, compares historical installer version/schema metadata, refuses repair downgrades/future schemas, takes a verified rollback backup before managed refresh, stages current installer-owned files, replays stale checkpoint revisions, refreshes declared managed pins, and runs policy-v11 hardware reconciliation. No intermediate release is required. Same-version repair remains local-first.
- Historical/versionless managed options in v14.5.46 were normalized through schema 20 without resetting user/application state. Pre-acceleration managed Ollama is normalized to explicit CPU ownership during cumulative migration rather than silently enabling a GPU path. Migration provenance is checkpoint-ephemeral.
- v14.5.42 advances adaptive resource policy to **v11**: compact/balanced/large WSL RAM profiles, compact/balanced/high CPU profiles, per-GPU NVIDIA/AMD VRAM topology, shared DirectML/Ollama VRAM coordination through `OLLAMA_GPU_OVERHEAD`, GPU-aware automatic context, and bounded runtime offload proof through `ollama ps`. Auto CPU fallback is fingerprinted; explicitly forced GPU modes fail closed when offload is not proven.
- v14.5.42 also freezes the finalized resource decision into one canonical policy object consumed by generated Compose, persisted state, verification, and `resource-policy-report.txt`. Separate hardware/policy SHA-256 fingerprints make repair drift diagnosable. v14.5.42 introduced the six-shard 135-fixture release contract; v14.5.43 extended it to 136 fixtures; v14.5.44 extended it to 137 fixtures; v14.5.45 extended it to 138 fixtures; v14.5.46 extended it to 139 fixtures and retains bytecode/cache/temp contamination rejection plus exact patch/ZIP round-trip requirements.
- v14.5.4 hardens the opt-in DirectML gateway with fail-closed dedicated-VRAM admission, adaptive effective context, low-CPU-memory model loading, and resource policy v10. DirectML host headroom is merged with the existing WSL reserve instead of double-counted; <=12 GiB WSL allocations receive tighter supporting-service ceilings while the proven Hermes floor remains. Ollama remains the embedding backend and automatic text fallback. Existing installs remain Ollama by default.
- v14.5.2 adds the install-preserving Option 7 cleanup/reclaim path and low-space Verify/Cleanup recovery gate while retaining all v14.5.1 runtime/resource-policy behavior unchanged.
- v14.5.1 adds resource policy v9 and running-container OOM detection while retaining the v14.5.0 read-only planning foundation. Policy v9 prevents the full-stack 544 MiB Hermes ceiling observed under policy v4 by providing a 1024 MiB preferred Hermes floor and 512 MiB Honcho API floor on sufficiently sized hosts, while keeping the aggregate managed budget bounded. The final v14.5.1 build persists generated per-service CPU/RAM ceilings and verifies effective Compose values against live Docker `HostConfig.Memory` and `HostConfig.NanoCpus`, covering both first container creation and existing-install repair convergence. Policy v9 also scales Hermes memory/CPU ceilings for additional persistent Matrix profile gateways and Kanban concurrency above 3, fingerprints that topology for repair/start refresh, and refuses selected-service/topology plans that cannot meet defined minimum memory ceilings instead of proportionally shrinking them into implausible hard limits. Generated root/startup helpers probe current CPU/RAM/topology at runtime.
- v14.5.0 adds a read-only WSL-native planning foundation without replacing the existing mutating reconciliation engine. `latticevale_readonly.py` parses installer selections/checkpoint state and treats `.env` as derived plus `compose.override.yaml` as opaque/user-owned; `repair-plan.py` powers `./manage.sh repair --plan`; `audit-free.py` reports the current free/local path; and `checkpoint-metadata.json` is a read-only mirror of the existing hardcoded stage revisions and CI verifies that the mirror stays synchronized; it is not an input to mutating repair. Windows PowerShell remains only for bootstrap and Windows-native integration/recovery responsibilities.
- v14.4.85 promotes the accumulated pre-release reliability candidate into a normal versioned release. It retains the v14.4.84 WSL shortcut/transport repair, adds startup-aware reconcile and post-gateway readiness barriers, verifies Synapse from inside `hermes-agent`, uses exact checked gateway lifecycle handling, and keeps internal `reconcile`/`kanban_gateway` checkpoints at revision 4 so existing v14.4.84 installs replay the corrected path. Option 6 now uses a bundle-owned pre-update safety-backup helper rather than the installed `manage.sh`, and Option 3 prints a fresh Linux + Windows read-only verification report.
...
- Adaptive per-service CPU/RAM ceilings are optional and `compose.override.yaml` remains the final user override layer. Current policy v13 keeps the aggregate-budget model but moves host reserve, DirectML reserve, CPU quotas, service-memory water filling, model/context/concurrency, GPU coordination, and allocator/database tuning into the canonical architecture library. Calculations scale from live WSL CPU/RAM, enabled service topology, actual model inputs, and measured backend/GPU/UMA capability rather than fixed machine-size bands. DirectML runtime health is fingerprinted separately from resource topology so safe Ollama fallback does not make a valid policy stale; resource-relevant managed-Ollama acceleration changes can still trigger rebudgeting. Repair/start automatically migrates enabled policy-v2 through policy-v12 overlays and refreshes when canonical hardware/topology/options inputs change.
- Current DirectML host-RAM budgeting is monotonic and topology-neutral: it scales to about 30% of live WSL RAM with bounded floor/cap behavior rather than a RAM tier or known-PC threshold. The pinned Qwen2/Qwen2.5 gateway applies a process-local `torch.where` mask compatibility shim for the tested torch-directml envelope, and fail-closed self-test errors now include bounded gateway diagnostics.
...
The Windows installer also exposes **Update / repair installer-managed software** for an existing managed stack. That mode requires a successful bundle-owned pre-update safety backup, bypasses the age gate, forces the current bundle's managed refresh, and then executes the normal stage verifiers. The backup helper is shipped with the installer rather than delegated to the currently installed `manage.sh`, so Update / repair can still repair an outdated/broken management script; failures report the exact backup step and captured process diagnostics before any software refresh begins. It is the reproducible way to apply component-version changes shipped by a newer LatticeVale bundle. Fixed pins such as Hermes/Synapse only advance when the bundle declares a different pin; installer-owned SearXNG/Ollama pins and the audited Honcho commit advance only across proven ownership boundaries. Explicit user overrides and separately owned native Windows Ollama remain outside that updater.
...
The Windows installer additionally exposes **Cleanup / reclaim disk space** as Option 7 for a recognized managed stack. Cleanup is isolated from repair/update reconciliation and streams the bundle-owned `linux/cleanup-storage.sh` helper to WSL root, avoiding extra staging inside a nearly-full distro. The user chooses one or more bounded categories before a final confirmation: exact-metadata Option 6 pre-update backups, stale root-owned installer staging, APT download cache, dangling Docker images, dangling default-builder cache, and WSL root TRIM. It never invokes Docker system/container/network/volume prune or `image prune -a`, never removes tagged images or configured models, and never deletes persistent application/database/vault/workspace/credential state. A recognized managed stack may reach Verify or Cleanup below the ordinary repair free-space floor; all existing mutating modes keep the normal storage gate, and fresh/unrecognized users cannot inherit the cleanup-only exception. Option 7 does not compact/move/resize the WSL VHDX.
