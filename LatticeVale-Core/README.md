...
- v14.4.84 Hotfix 2 fixes the startup-aware reconcile and post-gateway readiness races found during validation: it waits for managed Ollama and Matrix/Synapse readiness, reports exact failing components, verifies Hermes API/Dashboard after the final default-gateway mutation in both reconcile and Kanban/gateway stages, uses exact s6-scoped fallback, applies the same barrier to Start/Restart/Update, and advances both internal lifecycle checkpoints to revision 4 so Hotfix 1 installs replay the final corrected path during Resume / repair. VERSION remains 14.4.84.
