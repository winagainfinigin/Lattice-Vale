# LatticeVale v14.5.2 — Technical README
...
- Adaptive per-service CPU/RAM ceilings are optional and `compose.override.yaml` remains the final user override layer. Current policy v9 keeps the aggregate-budget model, protects managed Ollama, and corrects policy-v4 starvation by giving Hermes a 1024 MiB preferred floor and Honcho API 512 MiB on sufficiently sized hosts; CPU-backed managed Ollama uses a 4608 MiB provisional floor before model artifacts are measurable, then a model/context-aware floor after download and a 10% reserve on >6-12 GiB WSL allocations, while other >6-24 GiB shapes keep a 20% reserve. The 1024 MiB common Hermes floor covers the default gateway plus up to one secondary Matrix gateway and Kanban concurrency up to 3; extra secondary Matrix gateways add 192 MiB each beyond the first and Kanban slots above 3 add 96 MiB each, capped at 4096 MiB, while Hermes CPU can rise toward 100% under topology pressure. Repair/start automatically migrates enabled older policy overlays and refreshes when this topology changes.
