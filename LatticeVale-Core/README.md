# LatticeVale v14.5.46 — Technical README
...
- v14.5.42 also freezes the finalized resource decision into one canonical policy object consumed by generated Compose, persisted state, verification, and `resource-policy-report.txt`. Separate hardware/policy SHA-256 fingerprints make repair drift diagnosable. v14.5.42 introduced the six-shard 135-fixture release contract; v14.5.43 extended it to 136 fixtures; v14.5.44 extended it to 137 fixtures; v14.5.45 extended it to 138 fixtures; current v14.5.46 extends it to 139 fixtures and retains bytecode/cache/temp contamination rejection plus exact patch/ZIP round-trip requirements.
...
- Adaptive per-service CPU/RAM ceilings are optional and `compose.override.yaml` remains the final user override layer. Current policy v11 keeps the aggregate-budget model, protects managed Ollama, and retains the policy-v9 starvation correction with a 1024 MiB preferred Hermes floor and 512 MiB Honcho API floor on sufficiently sized hosts. CPU-backed managed Ollama now uses a 4096 MiB provisional floor before model artifacts are measurable, then a model/context-aware floor after download; GPU-backed managed Ollama records NVIDIA/AMD GPU count/min/max/aggregate VRAM, applies a per-GPU `OLLAMA_GPU_OVERHEAD` reserve when DirectML shares the same vendor, and uses a reduced CPU quota because inference is expected to be offloaded. The 1024 MiB common Hermes floor covers the default gateway plus up to one secondary Matrix gateway and Kanban concurrency up to 3; extra secondary Matrix gateways add 192 MiB each beyond the first and Kanban slots above 3 add 96 MiB each, capped at 4096 MiB, while Hermes CPU can rise toward 100% under topology pressure. Repair/start automatically migrates enabled policy-v2 through policy-v10 overlays and refreshes when hardware/topology or the recorded GPU-fallback fingerprint changes.
