...
- v14.3.41 adds a WSL cold-start host-safety boundary: normal installer flows no longer contain a mirrored-networking writer/fallback; existing mirrored mode is consumed only when already healthy and recorded as external/user-owned; the explicit host repair helper prioritizes a backed-up NAT recovery before DISM when E_UNEXPECTED coincides with mirrored mode. v14.3.40 changed documentation only. The inherited v14.3.39 existing-install QC removes automatic engine-global Docker image/build-cache pruning from repair maintenance; Docker disk usage remains visible, while automatic cleanup is limited to proven LatticeVale-owned disposable state so unrelated projects sharing the distro are preserved.
...
- `windows/LatticeVale-Shortcut.ps1` — inspectable current-user Start/Shutdown shortcut launcher; shutdown terminates only the selected distro.
- `tests/` — static and historical regression fixtures.

## Repair and update policy

Before a Windows Resume / repair or Update / repair run, the selected LatticeVale WSL distro should be fully stopped. When installed, **Shut Down LatticeVale** is the preferred preparation path because it stops the managed stack and then uses `wsl.exe --terminate <selected-distro>` for only that distro; global `wsl --shutdown` is unnecessary.
...
The Shut Down action does not issue global `wsl --shutdown`. It first checks `wsl --list --running`, returns successfully if that distro is already stopped, otherwise runs `./manage.sh stop` and then `wsl.exe --terminate <selected-distro>`. Same-name shortcuts are overwritten/removed only when their target and arguments prove they are installer-owned for the exact generated configuration.
...
- **Windows Tailscale** is an optional Windows-side integration. **Ubuntu Pro** is optional and external to the self-hosted application stack.
