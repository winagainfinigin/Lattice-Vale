# LatticeVale v14.4.82 — Technical README

- v14.4.82 fixes the v14.4.81 helper return channel: child PowerShell diagnostics are sent to `Out-Host` and only the scalar native exit code is returned, so a successful bounded WSL recovery actually enters the same-run eligibility re-probe. Recovery policy, storage thresholds, and ownership boundaries are unchanged.
