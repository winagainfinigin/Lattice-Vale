...
function Get-LatticeValeGpuAccelerationPlan([string]$Name, [string]$User) {
    ...
    if (($vendors -contains 'nvidia') -and $ollama.NvidiaWslReady) {
        ...
        $backend = 'directml'
        $ollamaAcceleration = 'auto'
        $reason = 'An NVIDIA GPU is visible through /dev/dxg but the CUDA/nvidia-smi WSL path is not verified. DirectML is the best available GPU attempt; its Ollama text fallback remains configurable.'
    }
...
function Select-LatticeValeDirectMLGpu(
    ...
    if (-not $wslState.WindowsBuildSupported) {
        ...
        $choice = Read-MenuExplicit 'DirectML is not supported by this Windows build. Choose how to continue' @(
            ...
            'Keep DirectML selected for a future Windows upgrade; the configured fallback policy will apply'
        )
    ...
    if ($wslState.TensorProbeAvailable) {
        if ($wslState.TensorProbeSucceeded) {
            ...
            Write-Warning "An existing isolated DirectML environment was found but its real tensor probe failed. Resume / repair will rebuild/retry the installer-owned environment and the configured Ollama fallback remains available when enabled.$(if ($wslState.TensorProbeDetail) { ' ' + $wslState.TensorProbeDetail } else { '' })"
        }
    }
...
if ($reusePriorChoices) {
    ...
    $useGpuAcceleration = [bool](Get-OptionValue $existingOptions 'useGpuAcceleration' $true)
    $directmlFallbackPolicy = [string](Get-OptionValue $existingOptions 'directmlFallbackPolicy' '')
    if ($directmlFallbackPolicy -notin @('managed','windows-native','none')) {
        # Schema <=22 DirectML always used the selected Ollama backend as fallback.
        $directmlFallbackPolicy = if ($localTextBackend -eq 'directml') { $ollamaBackend } else { 'none' }
    }
    ...
    if (-not $useGpuAcceleration) {
        $localTextBackend = 'ollama'
        $directmlFallbackPolicy = 'none'
    ...
    if ($installMode -eq 'change') {
        ...
        if ($changeScopes -contains 'local-ai') {
            ...
            if ($honcho -or $hermesLocalAI) {
                $gpuPlan = Get-LatticeValeGpuAccelerationPlan $DistroName $linuxUser
                Write-LatticeValeGpuAccelerationPlan $gpuPlan
                $useGpuAcceleration = Read-Choice 'Use GPU acceleration?' 'Allows LatticeVale to use a verified DirectML, CUDA, ROCm, Vulkan, or native-Windows Ollama GPU path according to the choices below. The selection is durable and Resume / repair preserves it.' 'Forces deliberate CPU-only local inference. LatticeVale will not provision or select GPU inference backends during this configuration.' $useGpuAcceleration
                if (-not $useGpuAcceleration) {
                    $localTextBackend = 'ollama'; $directmlFallbackPolicy = 'none'; $ollamaBackend = 'managed'; $ollamaAcceleration = 'cpu'; $persistOllamaAcceleration = $true
                    Write-Info 'GPU acceleration disabled by user policy; local inference will use managed Ollama on CPU.'
                }
                ...
                $directmlBackendLabel = 'PyTorch DirectML gateway (experimental; DirectX 12 GPU acceleration in WSL2; configurable Ollama fallback)' + $(if ($gpuPlan.RecommendedTextBackend -eq 'directml') { ' [recommended for detected hardware]' } else { '' })
                ...
                if ($localTextBackend -eq 'directml') {
                    Write-Info 'DirectML accelerates Hermes and Honcho text inference through one WSL-host gateway. Honcho embeddings may still use Ollama independently; text fallback is selected separately and can be disabled to fail closed.'
                ...
                while ($true) {
                    ...
                    $backendOptions = @("Use native Windows Ollama: $nativeStatus", 'LatticeVale-managed Ollama inside WSL/Docker')
                    if ($localTextBackend -eq 'directml') {
                        $noFallbackLabel = if ($honcho) { 'No Ollama text fallback - fail closed (Honcho embeddings still use managed WSL/Docker Ollama)' } else { 'No Ollama text fallback - fail closed' }
                        $backendOptions += $noFallbackLabel
                    }
                    $backendChoice = Read-Menu 'Where should Ollama run for its selected role?' $backendOptions $backendSuggested
                    if ($localTextBackend -eq 'directml' -and $backendChoice -eq 3) { $directmlFallbackPolicy = 'none'; $ollamaBackend = 'managed'; break }
                    if ($backendChoice -eq 2) { $ollamaBackend = 'managed'; $directmlFallbackPolicy = if ($localTextBackend -eq 'directml') { 'managed' } else { 'none' }; break }
                    ...
                    $ollamaBackend = 'windows-native'
                    $directmlFallbackPolicy = if ($localTextBackend -eq 'directml') { 'windows-native' } else { 'none' }
                    $windowsOllamaTransport = [string]$windowsNativeBridgeState.Transport
                    $windowsOllamaHostAddress = [string]$windowsNativeBridgeState.WindowsHostIp
                    $windowsOllamaTargetAddress = [string]$windowsOllamaState.RelayTargetAddress
                    $windowsOllamaTargetPort = [int]$windowsOllamaState.RelayTargetPort
                ...
                if ($localTextBackend -eq 'ollama' -or $directmlFallbackPolicy -ne 'none') {
                    while ($true) {
                ...
                if ($ollamaBackend -eq 'managed' -and -not $useGpuAcceleration) {
                    $ollamaAcceleration = 'cpu'; $persistOllamaAcceleration = $true
                } elseif ($ollamaBackend -eq 'managed') {
                    $gpuState = Get-OllamaWslGpuPrerequisites $DistroName $linuxUser
                    Write-OllamaGpuPrerequisiteSummary $gpuState
    ...
    if ($questionnaireMode -in @('custom','explicit')) {
        ...
        $hermesLocalAI = Read-Choice 'Use a LatticeVale local AI backend as the default Hermes AI provider?' 'Uses local inference for the main Hermes model. You can choose stable Ollama or the experimental PyTorch DirectML gateway; DirectML text fallback can be native Windows Ollama, managed WSL/Docker Ollama, or disabled to fail closed.' 'Hermes keeps the upstream interactive provider setup so you can choose an external or other provider.' ([bool](Get-OptionValue $old 'hermesLocalAI' $true))
        $localTextBackend = [string](Get-OptionValue $old 'localTextBackend' 'ollama')
        if ($localTextBackend -notin @('ollama','directml')) { $localTextBackend = 'ollama' }
        $useGpuAcceleration = [bool](Get-OptionValue $old 'useGpuAcceleration' $true)
        $savedFallbackPolicy = [string](Get-OptionValue $old 'directmlFallbackPolicy' '')
        if ($savedFallbackPolicy -notin @('managed','windows-native','none')) { $savedFallbackPolicy = 'managed' }
        $directmlFallbackPolicy = if ($localTextBackend -eq 'directml') { $savedFallbackPolicy } else { 'none' }
    ...
    $gpuPreferencePnpDeviceId = [string](Get-OptionValue $old 'gpuPreferencePnpDeviceId' '')
        ...
        if ($honcho -or $hermesLocalAI) {
            $gpuPlan = Get-LatticeValeGpuAccelerationPlan $DistroName $linuxUser
            Write-LatticeValeGpuAccelerationPlan $gpuPlan
            $useGpuAcceleration = Read-Choice 'Use GPU acceleration?' 'Allows LatticeVale to use a verified DirectML, CUDA, ROCm, Vulkan, or native-Windows Ollama GPU path. This durable choice is preserved by Resume / repair.' 'Forces deliberate CPU-only local inference; no GPU backend is selected or provisioned for local inference.' $useGpuAcceleration
            if (-not $useGpuAcceleration) {
                $localTextBackend = 'ollama'; $directmlFallbackPolicy = 'none'
                Write-Info 'GPU acceleration disabled by user policy; local inference will use managed Ollama on CPU.'
            }
            ...
            if ($localTextBackend -eq 'directml') {
                Write-Info 'DirectML is used for Hermes and Honcho text inference when healthy. Ollama text fallback is optional and can be disabled to fail closed; Honcho embeddings remain a separate Ollama role.'
                $directmlGpu = Select-LatticeValeDirectMLGpu $DistroName $linuxUser $directmlAdapterName $directmlGpuVendor $directmlVramMiB $gpuPreferenceId $gpuPreferencePnpDeviceId
                if (-not $directmlGpu.UseDirectML) {
                    $localTextBackend = 'ollama'
                    $directmlAdapterName = ''
        ...
        if ($honcho -or $hermesLocalAI) {
            ...
            while ($true) {
                ...
                $backendOptions = @("Use native Windows Ollama: $nativeStatus", $managedStatus)
                if ($localTextBackend -eq 'directml') {
                    $noFallbackLabel = if ($honcho) { 'No Ollama text fallback - fail closed (Honcho embeddings still use managed WSL/Docker Ollama)' } else { 'No Ollama text fallback - fail closed' }
                    $backendOptions += $noFallbackLabel
                }
                $backendChoice = Read-Menu 'Where should Ollama run for its selected role?' $backendOptions $backendSuggested
                if ($localTextBackend -eq 'directml' -and $backendChoice -eq 3) {
                    $directmlFallbackPolicy = 'none'; $ollamaBackend = 'managed'
                    Write-Info 'DirectML text fallback disabled. Text inference will fail closed if DirectML is unavailable; Honcho embeddings, when selected, remain independent.'
                    break
                }
                ...
                $ollamaBackend = 'managed'
                $directmlFallbackPolicy = if ($localTextBackend -eq 'directml') { 'managed' } else { 'none' }
        ...
        if (($honcho -or $hermesLocalAI) -and ($localTextBackend -eq 'ollama' -or $directmlFallbackPolicy -ne 'none')) {
            if ($ollamaBackend -eq 'windows-native') {
        ...
        $ollamaAcceleration = 'cpu'
        if (-not $useGpuAcceleration) {
            $ollamaBackend = 'managed'; $ollamaAcceleration = 'cpu'
        } elseif (($honcho -or $hermesLocalAI) -and $ollamaBackend -eq 'managed') {
...
$options = [ordered]@{
    ...
    directmlFallbackPolicy = if ($localTextBackend -eq 'directml') { $directmlFallbackPolicy } else { 'none' }
    ...
    inferenceBackendPreference = if (-not $useGpuAcceleration) { 'cpu' } elseif ($localTextBackend -eq 'directml') { 'directml' } elseif ($ollamaBackend -eq 'windows-native') { 'windows-native' } else { 'auto' }
