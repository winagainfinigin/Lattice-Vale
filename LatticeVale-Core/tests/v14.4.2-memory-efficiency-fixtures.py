...
assert "[POLICY_VERSION]=13" in cs
assert './configure-stack.sh --refresh-resource-policy' in boot
assert 'runtime-policy.py host-budget' in cs
assert 'runtime-policy.py service-plan' in cs
assert 'runtime-policy.py tuning' in cs
assert 'RESOURCE_POLICY_MODE' in cs and 'LOW_MEMORY_PROFILE' not in cs
...
# Current tuning is computed from live WSL RAM/CPU plus actual service allocations.
arch=(root/'stack/latticevale_arch.py').read_text()
for marker in ('def runtime_tuning(', 'def service_memory_plan(', 'def host_memory_budget('): assert marker in arch
