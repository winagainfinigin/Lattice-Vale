...
assert f"grep -q '^{version}$' LatticeVale-Core/VERSION.txt" in workflow
assert f"grep -q '^## {version} ' docs/CHANGELOG.md" in workflow
issue = (ROOT / ".github/ISSUE_TEMPLATE/bug_report.yml").read_text(encoding="utf-8")
assert f"placeholder: {version}" in issue
