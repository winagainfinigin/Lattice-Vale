...
from latticevale_arch import cpu_envelope_plan, cpu_quota_plan, host_memory_budget, service_memory_plan  # noqa:E402

assert '[POLICY_VERSION]=13' in cfg
...
for accel in ('cpu','vulkan','nvidia','amd'):
    ...
    for cpus in (1,2,3,5,7,11,19,37):
        ...
        cap = cpus * 1000
        envelope = cpu_envelope_plan(cpus, False)
        assert all(50 <= value <= cap for value in q.values())
        assert sum(q.values()) <= envelope['dockerEnvelopeMilli']
        assert q['hermes'] >= prev
        prev = q['hermes']
        busy = cpu_quota_plan(cpus, 8, 8, accel)
        assert busy['hermes'] >= q['hermes']
        assert all(50 <= value <= cap for value in busy.values())
        assert sum(busy.values()) <= envelope['dockerEnvelopeMilli']
