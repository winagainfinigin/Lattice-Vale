...
assert (ROOT / 'VERSION.txt').read_text(encoding='ascii').strip() in {'14.5.1','14.5.2','14.5.3','14.5.4','14.5.42','14.5.43','14.5.44','14.5.45','14.5.46','14.5.47','14.6.0','14.6.1'}
...
for accel in ('cpu','vulkan','nvidia','amd'):
    ...
    for cpus in (1,2,4,7,13,29):
        ...
        cap=cpus*1000
        assert all(50 <= value <= cap for value in base.values())
        assert all(50 <= value <= cap for value in busy.values())
        assert sum(base.values()) <= cap and sum(busy.values()) <= cap
...
for marker in (
    ...
    'policy v13 fingerprints already match',
):
