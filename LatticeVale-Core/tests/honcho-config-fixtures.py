...
m=re.search(r"python3 - \"\$local_text_model\" \"\$local_embedding_model\" \"\$\(ollama_openai_base_url\)\" <<'PY_HONCHO_LOCAL'\n(.*?)\nPY_HONCHO_LOCAL", text, re.S)
assert m, 'Honcho local config generator heredoc not found'
code=m.group(1)
with tempfile.TemporaryDirectory() as td:
    root=Path(td); (root/'config/honcho').mkdir(parents=True)
    r=subprocess.run(['python3','-','qwen3.5:4b','qwen3-embedding:4b','http://ollama:11434/v1'],input=code,text=True,cwd=root,capture_output=True)
