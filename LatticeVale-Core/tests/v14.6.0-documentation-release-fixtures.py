#!/usr/bin/env python3
"""v14.6.1 task-documentation and declarative-release-policy contract."""
from pathlib import Path
import json,re
ROOT=Path(__file__).resolve().parents[2]
assert (ROOT/'LatticeVale-Core/VERSION.txt').read_text().strip()=='14.6.1'
...
assert 'repository-only' in release.lower() and '14.6.1' in release
contrib=(ROOT/'docs/CONTRIBUTING.md').read_text()
assert 'Do not duplicate install-options schema ceilings' in contrib
workflow=(ROOT/'.github/workflows/validate.yml').read_text()
assert '14.6.1' in workflow
print('v14.6.1 documentation/release fixtures: PASS')
