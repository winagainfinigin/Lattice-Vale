...
release_install=(RELEASE_ROOT/'installer/install.ps1').read_text(encoding='utf-8')
...
check(len(_shipped_ps1)==12,f'expected 12 shipped PowerShell files, got {len(_shipped_ps1)}')
...
# Runtime variable safety: reject PowerShell automatic/read-only/constant clobbering and Bash readonly assignments.
r=subprocess.run([sys.executable,str(ROOT/'tests/runtime-variable-safety-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'runtime variable safety audit failed: ' + (r.stdout+r.stderr).strip())

r=subprocess.run([sys.executable,str(ROOT/'tests/permission-mode-policy-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'permission mode policy audit failed: ' + (r.stdout+r.stderr).strip())

r=subprocess.run([sys.executable,str(ROOT/'tests/v13.16.7-repair-semantics-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v13.16.7 repair semantics audit failed: ' + (r.stdout+r.stderr).strip())

r=subprocess.run([sys.executable,str(ROOT/'tests/v13.16.8-backup-recovery-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v13.16.8 backup recovery audit failed: ' + (r.stdout+r.stderr).strip())

r=subprocess.run([sys.executable,str(ROOT/'tests/v13.16.9-wsl-stderr-isolation-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v13.16.9 WSL stderr isolation audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v13.16.10-obsidian-bind-path-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v13.16.10 Obsidian bind-path audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v13.16.11-full-repair-audit-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v13.16.11 full repair reliability audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.1.3-profile-matrix-obsidian-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14 profile Matrix/Obsidian audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.1.3-windows-shortcuts-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.0 Windows shortcut audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.1.3-public-release-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.0 public release audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.1.3-portability-rebrand-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.0 portability/rebrand audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.0-usability-safety-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.0 usability/safety audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.5-native-windows-ollama-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.5 native Windows Ollama audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.6-matrix-first-bootstrap-pipefail-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.6 Matrix first-bootstrap/pipefail audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.7-profile-gateway-s6-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.7 exact-profile s6 gateway audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.8-matrix-v10-online-order-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.8 Matrix v10/online-order audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.9-profile-matrix-gateway-start-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.9 secondary Matrix manual-handoff audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.10-aged-repair-package-refresh-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.10 aged repair package-refresh audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.11-native-windows-ollama-detection-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.11 native Windows Ollama discovery audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.12-explicit-ollama-backend-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.12 explicit Ollama backend audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.13-powershell-source-encoding-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.13 PowerShell source encoding audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.14-native-ollama-questionnaire-remediation-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.14 native Ollama questionnaire remediation audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.15-wsl-native-ollama-topology-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.15 WSL/native Ollama topology audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.16-native-ollama-nat-adapter-scan-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.16 native Ollama NAT adapter/folder scan audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.17-wsl-direct-network-discovery-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.17 direct WSL network discovery audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.18-native-ollama-direct-wsl-fallback-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.18 native Ollama direct WSL fallback audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.19-native-ollama-wsl-firewall-rebind-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.19 native Ollama WSL firewall/rebind audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.20-native-ollama-mirrored-fallback-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.20 native Ollama mirrored fallback audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.21-uninstaller-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.21 uninstaller audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.22-native-ollama-http-lifecycle-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.22 native Ollama HTTP/lifecycle audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.23-public-customization-docs-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.23 public customization/docs audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.24-native-ollama-model-validation-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.24 native Ollama model validation audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.25-native-ollama-shortcut-matrix-pending-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.25 native Ollama shortcut/Matrix pending audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.26-native-relay-stabilization-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.26 native relay stabilization audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.27-uninstaller-single-distro-selection-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.27 uninstaller single-distro selection audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.28-uninstaller-recovery-discovery-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.28 uninstaller recovery discovery audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.29-uninstaller-native-ollama-readiness-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.29 uninstaller/native Ollama readiness audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.30-shared-wsl-network-policy-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.30 shared WSL networking policy audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.30-audit-wsl-host-preflight-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.30 audit WSL host preflight audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/wsl-preflight-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'functional WSL prerequisite decision audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.30-compatibility-port-reclaim-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.30 compatibility/stale bridge-port reclaim audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.30-matrix-profile-soft-pending-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.30 secondary Matrix soft-pending/resilient activation audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.37-update-repair-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.37 controlled Update / repair audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.38-kanban-skill-policy-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.38 Kanban/skill policy audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.39-existing-install-qc-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.39 existing-install QC audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.40-documentation-separation-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.3.40 documentation-separation audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.3.41-wsl-host-safety-fixtures.py')],capture_output=True,text=True)
...
check(r.returncode==0, 'v14.3.43 Scheduled Task action compatibility audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.4.0-documentation-audit-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.4.0 documentation audit failed: ' + (r.stdout+r.stderr).strip())
r=subprocess.run([sys.executable,str(ROOT/'tests/v14.4.5-repair-runtime-policy-update-fixtures.py')],capture_output=True,text=True)
check(r.returncode==0, 'v14.4.5 repair runtime-policy/update audit failed: ' + (r.stdout+r.stderr).strip())
...
for phrase in ('Install Hermes Dashboard?','Create multiple Hermes profiles?','Enable Hermes Kanban?','Install Matrix/Synapse?',
               ...
               'Install/configure Obsidian for Windows?','Install Ubuntu Pro for WSL?','Enable unattended Ubuntu security updates?',
...
if version == '14.4.7':
    check("integrations) printf '3' ;;" in configure,'v14.4.7 integration migration revision missing')
...
check(configure.index('Preparing selected Docker infrastructure before Hermes setup.') < configure.index('DEFAULT Hermes profile provider/model selection follows.'), 'supporting infrastructure must precede default Hermes provider/model selection')
check(configure.index('Matrix account setup: create one admin account and one Hermes bot account') < configure.index('DEFAULT Hermes profile provider/model selection follows.'), 'Matrix bootstrap must precede default Hermes provider/model selection')
check(ps1.index("Write-Step 'Bootstrapping Docker and the selected LatticeVale stack inside Ubuntu'") < ps1.index("Write-Step 'Installing selected Windows applications'"), 'Windows add-ons must follow successful Linux stack bootstrap')
