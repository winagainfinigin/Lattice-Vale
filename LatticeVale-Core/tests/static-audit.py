...
check('subprocess.Popen' in runner and 'start_new_session=True' in runner and 'tempfile.TemporaryFile' in runner, 'regression runner must execute isolated discovered fixtures')
...
check('local_text_openai_base_url' in configure and "f'base_url = {q(text_base)}'" in configure and "f'base_url = {q(embedding_base)}'" in configure,'Honcho must split selected text routing from Ollama embedding routing')
...
check('Use a LatticeVale local AI backend as the default Hermes AI provider?' in ps1,'Hermes local-AI choice missing')
...
check('schema = $compat.InstallOptionsSchema' in ps1,'current options schema must come from compatibility metadata')
...
check(version in {'14.6.0','14.6.1'}, 'release VERSION.txt must be 14.6.0')
for marker in ('INSTALL_OPTIONS_SCHEMA=23','HARDWARE_CAPABILITIES_SCHEMA=1','BACKEND_CAPABILITIES_SCHEMA=1','BACKEND_HEALTH_SCHEMA=1','RUNTIME_POLICY_SCHEMA=13','DIAGNOSTICS_SCHEMA=1'):
...
check('runtime-policy.py cpu-envelope' in configure and 'CPU_DOCKER_ENVELOPE_MILLI' in configure and 'CPU_DOCKER_ALLOCATED_MILLI' in configure, 'policy v13 conserved CPU envelope markers missing')
check('useGpuAcceleration' in ps1 and 'directmlFallbackPolicy' in ps1 and 'No Ollama text fallback - fail closed' in ps1, 'schema-23 GPU opt-out / DirectML fallback controls missing')
check('with TORCH.no_grad():' in read('stack/directml-gateway.py') and 'inference_mode()' not in read('stack/directml-gateway.py'), 'DirectML generation must use no_grad rather than inference_mode')
...
check("[version]'2.5.4'" in ps1 and 'instanceIdleTimeout=-1' in ps1 and 'vmIdleTimeout=-1' in ps1,'WSL 2.5.4+ persistent instance + VM lifetime policy missing')
check('function Set-WslGlobalIdleTimeoutsDisabled' in ps1 and 'function Get-WslGlobalVmIdleTimeout' in ps1,'WSL lifetime policy must manage both timers without touching unrelated configuration')
...
if version in {'14.4.7','14.4.8','14.4.81','14.4.82','14.4.83','14.4.84','14.4.85','14.5.0','14.5.1','14.5.2','14.5.3','14.5.4','14.5.42','14.5.43','14.5.44','14.5.45','14.5.46','14.5.47','14.6.0','14.6.1'}:
...
if version in {'14.4.82','14.4.83','14.4.84','14.4.85','14.5.0','14.5.1','14.5.2','14.5.3','14.5.4','14.5.42','14.5.43','14.5.44','14.5.45','14.5.46','14.5.47','14.6.0','14.6.1'}:
