...
assert "$ollamaTextRole = if ($localTextBackend -eq 'directml') { 'fallback' } else { 'text' }" in ps1
assert 'Local Ollama $ollamaTextRole model [suggested: $localTextModel; Enter accepts]' in ps1
assert 'DirectML Hugging Face text model [suggested: $directmlTextModel; Enter accepts]' in ps1
