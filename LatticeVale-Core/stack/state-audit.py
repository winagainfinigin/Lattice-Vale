...
def main() -> int:
    ...
    gpu_acceleration_enabled = bool(opts.get("useGpuAcceleration", True))
    ...
    directml_selected = local_ai and gpu_acceleration_enabled and local_text_backend == "directml"
    ...
    directml_fallback_policy = str(opts.get("directmlFallbackPolicy", "")).strip().lower()
    if directml_fallback_policy not in {"managed", "windows-native", "none"}:
        # Schema <=22 had no independent DirectML fallback field; migrate the
        # previous Ollama backend choice without changing historical behavior.
        directml_fallback_policy = ollama_backend if directml_selected else "none"
    text_fallback_enabled = directml_selected and directml_fallback_policy != "none"
    ollama_text_role = local_ai and (local_text_backend == "ollama" or text_fallback_enabled)
    ollama_embedding_role = selected("honcho")
    ollama_role = ollama_text_role or ollama_embedding_role
    effective_ollama_backend = directml_fallback_policy if text_fallback_enabled else ollama_backend
    if effective_ollama_backend not in {"managed", "windows-native"}:
        effective_ollama_backend = "managed"
    managed_ollama = ollama_role and effective_ollama_backend == "managed"
    native_ollama = ollama_role and effective_ollama_backend == "windows-native"
    ...
    if ollama_text_role:
        required_ollama_models.add(str(opts.get("localTextModel", "")).strip())
    if ollama_embedding_role:
        required_ollama_models.add(str(opts.get("localEmbeddingModel", "")).strip())
    required_ollama_models.discard("")
    ollama_storage_configured = ((root / "data/ollama").exists() if managed_ollama else (root / ".windows-native-info").is_file())
    ollama_configured = bool(ollama_role and required_ollama_models and ollama_storage_configured)
    ...
    if managed_ollama and docker_available and not args.offline:
        ...
        if ollama_running:
            ...
            needed = set(required_ollama_models)
    ...
    if not ollama_role:
        ...
        detail = "no selected Ollama role; DirectML text inference is fail-closed" if directml_selected else "not selected"
    elif native_ollama:
        ...
        roles = []
        if ollama_text_role:
            roles.append("text/fallback")
        if ollama_embedding_role:
            roles.append("embeddings")
        detail = "native Windows Ollama via WSL-only relay; role=" + "+".join(roles) + "; models remain in the Windows runtime"
        if native_ollama_detail:
            detail += f" ({native_ollama_detail})"
        if ollama_running and not ollama_models_ok:
            detail += "; one or more selected-role models are missing"
    else:
        ollama_broken = managed_ollama and docker_available and ollama_configured and not args.offline and not (ollama_running and ollama_models_ok) and runtime_probe_failed(ollama_state)
        ollama_status = classify_runtime(managed_ollama, ollama_configured, ollama_running and ollama_models_ok, ollama_state, ollama_broken)
        roles = []
        if ollama_text_role:
            roles.append("text/fallback")
        if ollama_embedding_role:
            roles.append("embeddings")
        detail = "managed Ollama role=" + "+".join(roles)
        if ollama_status == "STARTING":
            detail += "; container is still starting"
    ...
    if not directml_selected:
        ...
        if not directml_configured:
            ...
            if rc == 0:
                ...
                reported_fallback_enabled = dmh.get("fallback_enabled") is True
                if dml_ready or (text_fallback_enabled and fb_ready):
                    ...
                    if text_fallback_enabled:
                        directml_detail = f"gateway healthy; model={directml_model}; DirectML={'ready' if dml_ready else 'degraded'}; Ollama fallback={'ready' if fb_ready else 'unavailable'} ({directml_fallback_policy})"
                    else:
                        directml_detail = f"gateway healthy; model={directml_model}; DirectML=ready; Ollama text fallback disabled (fail closed)"
                    if last_backend:
                        directml_detail += f"; last inference={last_backend}"
                else:
                    directml_status = "BROKEN"
                    if text_fallback_enabled:
                        directml_detail = "gateway responds but neither DirectML nor its configured Ollama text fallback is ready"
                    else:
                        directml_detail = "gateway responds but DirectML is not ready; Ollama text fallback is disabled by fail-closed policy"
                if reported_fallback_enabled != text_fallback_enabled:
                    directml_status = "PARTIAL"
                    directml_detail += "; gateway fallback state disagrees with install-options policy"
            else:
                directml_status = "BROKEN"
    ...
    for key, label, container, url, cfgpath in service_defs:
        ...
        if key == "honcho" and enabled:
            if directml_selected:
                text_route = "DirectML gateway with " + (("native Windows Ollama" if directml_fallback_policy == "windows-native" else "managed WSL/Docker Ollama") + " fallback" if text_fallback_enabled else "fail-closed no-Ollama fallback")
            else:
                text_route = "native Windows Ollama" if native_ollama else "local WSL/Docker Ollama"
            embedding_route = "native Windows Ollama" if native_ollama else "local WSL/Docker Ollama"
            detail = ("fully local Honcho + pgvector/Redis; text routed to " + text_route + "; embeddings routed to " + embedding_route) + ("; containers are still starting" if status == "STARTING" else ("; containers are stopped" if status == "STOPPED" else ""))
