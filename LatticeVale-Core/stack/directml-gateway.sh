...
directml_fallback_policy() {
  local v b
  v="$(opt_text directmlFallbackPolicy)"
  if [[ "$v" != managed && "$v" != windows-native && "$v" != none ]]; then
    b="$(opt_text ollamaBackend)"; [[ "$b" == windows-native ]] && v=windows-native || v=managed
  fi
  printf '%s' "$v"
}
text_fallback_enabled() { [[ "$(directml_fallback_policy)" != none ]]; }
directml_selected() { gpu_acceleration_enabled && [[ "$(local_text_backend)" == directml ]] && [[ "$(jq -r '(.honcho // false) or (.hermesLocalAI // false)' install-options.json)" == true ]]; }
...
ollama_backend() { local v; v="$(directml_fallback_policy)"; [[ "$v" == managed || "$v" == windows-native ]] || v=managed; printf '%s' "$v"; }
adapter_name() {
  local v
...
write_force_fallback() {
  ...
  if ! text_fallback_enabled; then
    ...
    log_msg "refusing forced Ollama fallback marker because directmlFallbackPolicy=none: $reason"
    return 1
  fi
...
reconcile_force_fallback() {
  if ! text_fallback_enabled; then
    [[ -e "$force_fallback_file" ]] && log_msg 'removing legacy force-fallback marker because directmlFallbackPolicy=none'
    rm -f "$force_fallback_file"
    return 0
  fi
...
install_dependencies() {
  ...
  if [[ ! -x "$venv/bin/python" ]]; then
    ...
    if ! python3 -m venv "$venv"; then
      echo 'WARNING: Could not create the isolated DirectML Python venv; repair will retry.' >&2
      printf '%s\n' 'python3 -m venv failed' >"$deps_failed"
      text_fallback_enabled && return 0 || return 1
    fi
  fi
...
PY_DML_ENV_SHAPE
  ...
  if [[ "$rebuild" == true ]]; then
    ...
    rm -rf "$venv"
    python3 -m venv "$venv" || { printf '%s\n' 'python3 -m venv failed during DirectML dependency migration' >"$deps_failed"; text_fallback_enabled && return 0 || return 1; }
    current=''
  fi
...
PY_VERIFY_DML
  ...
  printf '%s\n' 'Pinned vendor-neutral DirectML dependency installation or tensor probe failed.' >"$deps_failed"
  if text_fallback_enabled; then
    echo 'WARNING: PyTorch DirectML could not be activated. LatticeVale will use the selected Ollama fallback; Resume / repair will retry DirectML installation.' >&2
    return 0
  fi
  echo 'ERROR: PyTorch DirectML could not be activated and Ollama text fallback is disabled; failing closed.' >&2
  return 1
}
...
start_worker() {
  ...
  if text_fallback_enabled && [[ "$(ollama_backend)" == windows-native ]]; then native_url="$(native_fallback_url 2>/dev/null || true)"; fi
  ...
  log_msg "starting DirectML gateway worker listen=${host}:$(port) model=$(model) fallback_policy=$(directml_fallback_policy) cpu_threads=${cpu_threads} host_reserve=${host_reserve}MiB"
  env \
    ...
    LATTICEVALE_DIRECTML_FORCE_FALLBACK="$(if text_fallback_enabled && [[ -e "$force_fallback_file" ]]; then printf 1; else printf 0; fi)" \
    LATTICEVALE_DIRECTML_FALLBACK_POLICY="$(directml_fallback_policy)" \
...
supervise_gateway() {
  ...
  while [[ ! -e "$disabled_file" ]]; do
    ...
    if start_worker; then
        ...
        failures=$((failures+1))
        if (( failures >= 2 )) && text_fallback_enabled && [[ ! -e "$force_fallback_file" ]]; then
          log_msg 'DirectML worker failed twice before reaching health; switching to configured Ollama fallback to protect WSL host resources'
          write_force_fallback 'repeated DirectML worker startup failure'
          failures=0; delay=5
        else
          if ! text_fallback_enabled; then rm -f "$force_fallback_file"; log_msg 'DirectML worker failure remains fail-closed because text fallback is disabled'; fi
...
PY_SELF_PAYLOAD
  ...
  local curl_error_file="$state_dir/self-test-curl-error.tmp"
  rm -f "$curl_error_file"
  if ! response="$(timeout --foreground --kill-after=20s 3600s curl -fsS --noproxy '*' --connect-timeout 5 --max-time 3500 -H 'Content-Type: application/json' -d "$payload" "http://${host}:$(port)/v1/chat/completions" 2>"$curl_error_file")"; then
    echo 'DirectML self-test HTTP request failed. Gateway diagnostics follow:' >&2
    [[ -s "$curl_error_file" ]] && { echo '--- curl ---' >&2; cat "$curl_error_file" >&2; }
    if [[ -s "$log_file" ]]; then
      echo '--- last 120 lines of logs/directml-gateway.log ---' >&2
      tail -n 120 "$log_file" >&2 || true
    else
      echo 'DirectML gateway log is empty or unavailable.' >&2
    fi
    rm -f "$curl_error_file"
    ...
    if ! text_fallback_enabled; then
      rm -f "$force_fallback_file"
      python3 latticevale_arch.py record-health directml failed DML_RUNTIME_FAILED --stack . --compat compatibility.conf --hardware data/latticevale/hardware-capabilities.json --detail "DirectML model self-test terminated without an HTTP response; fail-closed fallback policy" >/dev/null 2>&1 || true
      echo 'ERROR: DirectML model self-test terminated without an HTTP response and Ollama text fallback is disabled. Failing closed; Resume / repair will retry DirectML.' >&2
      return 1
    fi
    echo 'WARNING: DirectML model self-test terminated without an HTTP response. Switching this installation to the configured Ollama fallback until the next Resume / repair retries DirectML.' >&2
