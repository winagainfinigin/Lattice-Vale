...
gpu_acceleration_enabled() {
  # Schema <=22 did not persist this key; migration preserves the historical
  # GPU-enabled behavior unless the user explicitly opts out in schema 23.
  jq -e '(.useGpuAcceleration // true) == true' install-options.json >/dev/null 2>&1
}
...
directml_fallback_policy() {
  local value backend
  value="$(opt_text directmlFallbackPolicy)"
  if [[ "$value" != managed && "$value" != windows-native && "$value" != none ]]; then
    # Schema <=22 DirectML always used the selected Ollama backend as fallback.
    if [[ "$(local_text_backend)" == directml ]]; then
      backend="$(opt_text ollamaBackend)"
      [[ "$backend" == windows-native ]] && value=windows-native || value=managed
    else
      value=none
    fi
  fi
...
directml_text_enabled() {
  local_ai_enabled && gpu_acceleration_enabled && [[ "$(local_text_backend)" == directml ]]
}

text_fallback_enabled() {
  directml_text_enabled && [[ "$(directml_fallback_policy)" != none ]]
}

ollama_text_role_enabled() {
  local_ai_enabled || return 1
  if ! gpu_acceleration_enabled; then return 0; fi
  [[ "$(local_text_backend)" == ollama ]] && return 0
  directml_text_enabled && [[ "$(directml_fallback_policy)" != none ]]
}

ollama_embedding_role_enabled() {
  [[ "$(opt_bool honcho)" == true ]]
}

ollama_role_enabled() {
  ollama_text_role_enabled || ollama_embedding_role_enabled
}
...
managed_ollama_enabled() {
  ollama_role_enabled && [[ "$(ollama_backend)" == managed ]]
}

windows_native_ollama_enabled() {
  ollama_role_enabled && [[ "$(ollama_backend)" == windows-native ]]
}
...
ollama_gpu_inventory() {
  ...
  # Resource policy v13 deliberately inventories individual adapters instead of
...
resolve_ollama_acceleration() {
  ...
  if ! gpu_acceleration_enabled; then printf '%s' cpu; return 0; fi
...
resource_directml_selected() {
  ...
  jq -e '((.honcho // false) or (.hermesLocalAI // false)) and ((.useGpuAcceleration // true) == true) and ((.localTextBackend // "ollama") == "directml")' install-options.json >/dev/null 2>&1
}
  ...
  if [[ ! -s "$state" || -L "$state" ]]; then
    {
      ...
      printf 'Generated by: LatticeVale v14.6.1\n'
    } > "$report"
    chmod 0644 "$report"
    return 0
  fi
...
write_latticevale_compose_overlay() {
  ...
  local cpu_plan cpu_envelope_plan tuning_plan resource_plan active_count=0
  local malloc_arena_max synapse_cache_factor pg_shared_buffers
  local cpu_system_headroom_milli cpu_directml_reserve_milli cpu_docker_envelope_milli cpu_docker_allocated_milli=0
  declare -A cpu_limits=()
  cpu_plan="$(python3 runtime-policy.py cpu-plan "$cpus" "$matrix_profile_gateways" "$kanban_concurrency" "$accel" \
    "$(opt_bool matrix)" "$(opt_bool searxng)" "$(opt_bool qmd)" \
    "$(if managed_ollama_enabled; then printf true; else printf false; fi)" "$(opt_bool honcho)" "$directml_selected_for_budget")" || return 1
  while IFS='=' read -r svc milli; do
    [[ -n "$svc" && "$milli" =~ ^[0-9]+$ && "$milli" -ge 50 ]] || continue
    cpu_limits["$svc"]="$milli"
    cpu_docker_allocated_milli=$((cpu_docker_allocated_milli+milli))
  done <<<"$cpu_plan"
  cpu_envelope_plan="$(python3 runtime-policy.py cpu-envelope "$cpus" "$directml_selected_for_budget")" || return 1
  IFS=: read -r cpu_system_headroom_milli cpu_directml_reserve_milli cpu_docker_envelope_milli <<<"$cpu_envelope_plan"
  [[ "$cpu_system_headroom_milli" =~ ^[0-9]+$ && "$cpu_directml_reserve_milli" =~ ^[0-9]+$ && "$cpu_docker_envelope_milli" =~ ^[0-9]+$ ]] || { echo 'Canonical CPU envelope planner returned invalid values.' >&2; return 1; }
  (( cpu_docker_allocated_milli <= cpu_docker_envelope_milli )) || { echo 'Canonical CPU planner exceeded the conserved Docker CPU envelope.' >&2; return 1; }
  ...
  if [[ "$limits" == true ]]; then
    ...
    local expected_cpu_service
    for expected_cpu_service in "${!mem_limits[@]}"; do
      [[ "${cpu_limits[$expected_cpu_service]:-}" =~ ^[0-9]+$ ]] || { echo "Canonical CPU planner omitted enabled service '$expected_cpu_service'." >&2; return 1; }
    done
    for expected_cpu_service in "${!cpu_limits[@]}"; do
      [[ -n "${mem_limits[$expected_cpu_service]:-}" ]] || { echo "Canonical CPU planner emitted disabled service '$expected_cpu_service'." >&2; return 1; }
    done
  fi
  ...
  # Policy v13 canonicalization: after hardware/topology/model planning completes,
  ...
  declare -A resource_policy=(
    ...
    [CPU_SYSTEM_HEADROOM_MILLI]="$cpu_system_headroom_milli" [CPU_DIRECTML_RESERVE_MILLI]="$cpu_directml_reserve_milli"
    [CPU_DOCKER_ENVELOPE_MILLI]="$cpu_docker_envelope_milli" [CPU_DOCKER_ALLOCATED_MILLI]="$cpu_docker_allocated_milli"
  ...
  if [[ "$limits" == true ]]; then
    {
      # Keep POLICY_VERSION=13 first for human readability and compatibility with
...
choose_ollama_context_length() {
  # Policy v13 sizes the automatic context from both WSL RAM and the *usable* VRAM
...
if local_ai_enabled; then
  if ollama_text_role_enabled; then
    ...
    [[ -n "$local_text_model" ]] || { echo 'Invalid options: localTextModel is required when Ollama has a text-generation role.' >&2; exit 3; }
  fi
...
reconcile_model_aware_ollama_resources() {
  ...
  if [[ "$before" != "$after" ]]; then
    echo 'Managed Ollama model artifacts are now measurable; reconciling the model-aware policy v13 ceiling before model inference/embedding verification.'
    timeout --foreground --kill-after=10s 180s docker compose up -d --pull never --no-build ollama || return 1
    wait_managed_ollama_healthy 60 || return 1
...
if local_ai_enabled; then
  echo 'Quiescing existing Hermes/Honcho model consumers before local-model validation.'
  timeout --foreground --kill-after=10s 90s docker compose stop hermes honcho-api honcho-deriver >/dev/null 2>&1 || true

  if ollama_role_enabled; then
    if managed_ollama_enabled; then
    ...
    if managed_ollama_enabled; then
      ...
      verification_model=''
      if ollama_text_role_enabled; then
        verification_model="$(opt_text localTextModel)"
      elif [[ "$(opt_bool honcho)" == true ]]; then
        verification_model="$(opt_text localEmbeddingModel)"
      fi
      if [[ -n "$verification_model" ]]; then
        verify_or_rebudget_managed_ollama_acceleration "$current_managed_accel" "$verification_model" || return 1
      fi
    fi
...
if [[ "$(opt_bool hermesLocalAI)" == true ]]; then
  ...
  # contract. DirectML is a host gateway; Ollama text fallback is used only when the durable fallback policy enables it.
...
if local_ai_enabled; then
  ...
  if [[ "$runtime_fallback_active" == true || "$active_text_backend" != "$configured_text_backend" ]]; then
    ...
    if [[ "$active_text_backend" == unavailable ]]; then
      echo "  Text backend runtime: unavailable (fail closed; reason=$runtime_reason)"
    else
      echo "  Text backend runtime: $active_text_backend (configured fallback active; reason=$runtime_reason)"
    fi
  else
  ...
  if directml_text_enabled; then
    ...
    if [[ "$dml_admission_mib" =~ ^[0-9]+$ && "$dml_admission_mib" -ge 256 ]]; then
      ...
      if text_fallback_enabled; then
        echo '  DirectML bounded admission capacity: unavailable; model admission will use the configured Ollama text fallback.'
      else
        echo '  DirectML bounded admission capacity: unavailable; Ollama text fallback is disabled, so admission fails closed.'
      fi
    fi
    if text_fallback_enabled; then
      echo "  Ollama text fallback: $(directml_fallback_policy); model=$(opt_text localTextModel)"
    else
      echo '  Ollama text fallback: disabled (fail closed)'
    fi
  elif ollama_text_role_enabled; then
    echo "  Ollama text model: $(opt_text localTextModel)"
  fi
  if ollama_role_enabled; then
    echo "  Ollama selected-role acceleration: $(sed -n 's/^LATTICEVALE_OLLAMA_ACCELERATION=//p' .env | head -n1)"
  else
    echo '  Ollama selected role: none'
  fi
