...
v14.6.0 consolidates the hardware/backend/resource-policy layers introduced and hardened across 14.5.x. Durable user choices remain in `install-options.json`; machine-derived Windows/WSL hardware, backend capability/health/selection, and runtime policy are stored separately under `data/latticevale/` with schemas, fingerprints, atomic writes, and canonical validation. Repair now follows the dependency chain **options → hardware → backend capability/health → resource policy → generated runtime configuration** instead of independently recreating the same assumptions in multiple components.

GPU qualification is capability-driven and cross-vendor: DirectML, CUDA, ROCm, Vulkan, native-Windows Ollama, and CPU are classified independently. WSL-host DirectML remains valid independently of Linux-native GPU enumeration, explicit adapter intent is preserved across hardware changes, and runtime failures are fingerprinted rather than treated as permanent machine identity. CPU remains a first-class safe backend for otherwise-qualified systems.

**Same-version 14.6.0 compatibility hardening:** Windows GPU memory is now derived from PNP-correlated DXDiag dedicated/shared-memory data plus the 64-bit display-driver registry value, with legacy WMI/DWORD values accepted only as conservative lower bounds. The selected adapter receives a bounded DirectML admission capacity even when `torch_directml.gpu_memory()` is unavailable. UMA/integrated adapters keep shared memory distinct from dedicated VRAM and receive an adaptive WSL-RAM-bounded admission ceiling rather than a discrete-GPU assumption. Transient DirectML model/runtime failure can activate Ollama fallback without changing the resource-policy backend fingerprint, so a safe fallback no longer makes an otherwise valid repair fail final policy verification. Installer Option 8 stages the current canonical validator and a fresh Windows hardware snapshot even when auditing a pre-14.6 managed stack.

LatticeVale owns the installer-managed DirectML Python environment, backend probes, adapter correlation, admission/fallback policy, and derived runtime policy. Windows/WSL itself and the installed vendor display driver remain host prerequisites outside LatticeVale ownership. Microsoft-supported WSL DirectML is evaluated independently from CUDA/ROCm/Vulkan; an AMD, Intel, NVIDIA, Qualcomm, discrete, integrated, single-GPU, or multi-GPU system is never accepted or rejected solely by vendor/model name.

Start here: [Quickstart](docs/QUICKSTART.md) · [Installation](docs/INSTALLATION.md) · [Repair](docs/REPAIR.md) · [GPU backends](docs/GPU-BACKENDS.md) · [Diagnostics](docs/DIAGNOSTICS.md) · [Resource policy](docs/RESOURCE-POLICY.md) · [Architecture](docs/ARCHITECTURE.md) · [GitHub packaging](docs/GITHUB-REPOSITORY.md). Historical v13 patch-note archives remain in the repository but are repository-only under the 14.6 release-content policy and are omitted from normal end-user release ZIPs.

### v14.5.47 — WSL GPU recovery and cross-vendor fallback

**Same-version Hotfix 1 (2026-09-04):** fixes an Option 1 **Resume / repair installation** abort introduced by the initial 14.5.47 package. The Windows installer correctly normalizes managed options to schema 21, but the Linux `configure-stack.sh` validator was accidentally left capped at schema 20, so repair could reject the installer's own newly written `install-options.json`. The validator now accepts current schema 21 (and `repairOriginSchema` through 21) while continuing to reject future schema 22+. The release identity remains **14.5.47**. Users of the initial 14.5.47 ZIP should replace it with this revised full release and rerun Option 1; no uninstall, distro recreation, or data reset is required.

**Same-version Hotfix 2 (2026-09-04):** fixes a later Option 1 **Resume / repair** failure in resource-policy reconciliation. The policy writer used the current DirectML host-memory rule (25% of WSL RAM, bounded to 2-4 GiB) while the verifier still re-derived an older fixed-tier reserve, so it could reject the policy it had just generated. Generation and verification now use one canonical `resource_host_memory_budget()` calculation and verify the total host reserve plus container budget as well as the DirectML sub-reserve. This is topology-driven: DirectML through `/dev/dxg` remains valid independently of Linux-native CUDA/ROCm/DRM GPU enumeration, and CPU, Vulkan, NVIDIA, AMD/ROCm, zero-Linux-GPU, and multi-GPU managed-Ollama cases retain their own safe paths. Version remains **14.5.47**.

v14.5.47 makes the selected Windows GPU identity authoritative earlier in the DirectML startup path. The WSL-host DirectML probes and worker now export `MESA_D3D12_DEFAULT_ADAPTER_NAME` before importing `torch_directml`, then retain the existing exact Python-side adapter check. The installer records dedicated VRAM from Windows DXDiag for the selected adapter and the gateway uses that value only when the runtime cannot report usable dedicated memory; model admission remains fail-closed when neither source is trustworthy.

DirectML hard-fallback state is now tied to a WSL/GPU/runtime fingerprint. A legacy v14.5.46 marker or a changed kernel/bridge/adapter/runtime fingerprint receives one fresh DirectML retry, while repeated failures on the same runtime shape continue using Ollama rather than hot-looping. `directml-gateway.sh diagnose` and the new read-only `LatticeVale-Core/tools/Audit-LatticeVale-Gpu.ps1` expose the selected adapter, WSL bridge, tensor probe, fallback state, and Docker-to-host routing without modifying the installation.

Managed WSL/Docker Ollama also gains an explicit **Vulkan** acceleration path for systems that expose `/dev/dri/renderD*` but do not have a verified NVIDIA CUDA or AMD ROCm container path. LatticeVale passes the existing DRM devices to the standard pinned Ollama image with `OLLAMA_VULKAN=1` and accepts GPU-sized execution only after real model offload is proven through `ollama ps`. NVIDIA CUDA, AMD ROCm, DirectML, native-Windows Ollama, and CPU fallback remain available; LatticeVale still does not install or replace Windows display drivers.

For the historical v14.5.47 release, the options schema was 21, managed repair refresh revision was 3, and the release contract was **140 deterministic fixtures**. LatticeVale no longer offers or manages Ubuntu Pro for WSL; unrelated external Ubuntu Pro state remains untouched.

### v14.5.46 — GPU-aware onboarding and prerequisite provisioning

v14.5.46 made GPU acceleration part of the normal installer questionnaire instead of assuming users already know which WSL runtime path fits their hardware. It inventories recognized Windows AMD/NVIDIA/Intel/Qualcomm adapters, directly checks WSL DirectML and managed-Ollama GPU prerequisites, reports which installer-owned WSL components already exist, and marks the recommended local text backend/managed-Ollama acceleration option without removing explicit user choice.
...
LatticeVale resource policy **v13** keeps hard per-container ceilings while conserving one WSL-visible CPU envelope across all enabled Docker services and a bounded aggregate managed-container RAM budget. Host reserve, DirectML host reserve, container RAM budget, per-service memory, CPU quotas, model/context limits, and runtime tuning are derived continuously from the CPU/RAM WSL actually exposes plus enabled services and backend/model evidence. Hermes retains its topology-aware safety floor; managed Ollama uses a provisional model allowance derived from current RAM/context and replaces it with measured model-artifact sizing after download. Policy v13 also inventories eligible NVIDIA/AMD GPU topology, coordinates same-vendor Ollama/DirectML VRAM with per-GPU `OLLAMA_GPU_OVERHEAD`, and verifies post-load GPU offload before keeping GPU-sized resource assumptions. If selected service/model minima cannot fit inside the current budget, v13 refuses the unsafe plan and reports the constraint instead of silently overcommitting the VM.
When DirectML is selected, current v13 raises WSL-host RAM headroom smoothly to about 30% (bounded by an affordable 2 GiB floor and an 8 GiB cap) without fixed RAM tiers; the container budget is monotonic as WSL RAM increases. The managed Qwen2/Qwen2.5 DirectML path also carries a version-gated `torch.where` mask compatibility shim and reports a bounded gateway-log tail when the model self-test terminates before returning HTTP.

Depending on the enabled services, it can apply:

- adaptive per-container CPU and memory ceilings
- continuous WSL-aware host/container budgeting from the RAM actually visible to WSL, with bounded safety floors/caps and a separate adaptive DirectML WSL-host reserve when that backend is configured
- reduced glibc allocator arenas
- Synapse cache tuning
- reduced PostgreSQL fixed `shared_buffers`
- protected managed-Ollama memory headroom within the aggregate WSL container budget, including RAM/context-derived provisional sizing and measured model-aware post-download sizing
...
Clean installs generate policy v13 before first container creation. Existing adaptive policy-v2 through policy-v12 installs regenerate through normal Resume / repair or managed start-time resource reconciliation. Policy v13 fingerprints resource-relevant hardware/topology plus durable backend intent; transient DirectML runtime-health/fallback evidence has a separate fingerprint so safe gateway fallback does not falsely stale an otherwise valid policy. Hardware changes or resource-relevant CUDA/ROCm/Vulkan/CPU execution changes still invalidate dependent generated ceilings. Final reconciliation compares effective merged Compose `mem_limit`/`cpus` with Docker's live `HostConfig.Memory`/`HostConfig.NanoCpus`. Explicit user `compose.override.yaml` configuration remains authoritative and is applied after generated LatticeVale resource policy. LatticeVale does not write global WSL `memory` or `autoMemoryReclaim` settings. Generated root/Windows-start helpers evaluate current CPU, RAM, enabled-limit state, profile topology, Kanban concurrency, and acceleration state when the helper runs rather than freezing install-time values.
