## v14.6.1 architecture support

## Current policy-13 support note

CPU-only operation is supported and persists through repair. DirectML fallback can be native Windows Ollama, managed WSL/Docker Ollama, or disabled. Disabled text fallback is intentionally fail closed even if Honcho still uses Ollama for embeddings. Policy v13 prevents aggregate Docker CPU ceilings from exceeding the WSL-visible Docker envelope and is regenerated when relevant resources/topology change.

Use `./manage.sh diagnose`, `diagnose-gpu`, `diagnose-backends`, and `diagnose-policy` to separate Windows GPU identity, WSL topology, backend availability/health, and resource-policy problems. Existing-stack installer diagnostics are read-only. A failed optional GPU backend does not by itself make an otherwise-qualified CPU-capable installation unsupported.

## v14.5.47 WSL GPU recovery support

## v14.5.47 Resume / repair resource-policy hotfix

If Option 1 previously stopped at `Adaptive runtime/RAM policy regeneration completed but failed live verification`, replace the release files with the revised v14.5.47 package and rerun Option 1. The failure could be caused by an internal writer/verifier mismatch, not by insufficient hardware: DirectML policy generation used the current bounded 25% WSL-host reserve while verification used an older tier table. The revised package uses one canonical calculation.

A DirectML-selected WSL installation can legitimately report zero Linux-native GPU adapters in the managed-Ollama resource inventory. DirectML uses WSL's `/dev/dxg`/D3D12 bridge; managed Ollama separately requires the runtime appropriate to its selected acceleration (CUDA, ROCm, Vulkan/DRM, or CPU). Zero Linux-native adapters therefore means the managed Ollama path may fall back to CPU; it does not by itself invalidate DirectML.


For DirectML failures, run `cd ~/hermes-stack && ./directml-gateway.sh diagnose` when the stack is in the default location, or run `LatticeVale-Core/tools/Audit-LatticeVale-Gpu.ps1` from Windows PowerShell to discover the managed stack and collect read-only Windows/WSL/DirectML/Docker routing state. The audit never installs packages, clears markers, restarts services, or changes GPU drivers.

A legacy v14.5.46 DirectML `force-fallback` marker is retried once on v14.5.47. Thereafter the marker is tied to the relevant kernel/bridge/selected-adapter/runtime fingerprint; unchanged repeated failures stay on Ollama fallback, while a material runtime change can retry. If DirectML cannot report VRAM, the selected adapter's installer-recorded DXDiag dedicated memory may be used for admission. If neither runtime nor installer has trustworthy capacity, DirectML still refuses model placement.

Managed Ollama Vulkan is available when WSL exposes `/dev/dri/renderD*`. It is not equivalent to ROCm: LatticeVale passes the existing DRM devices to the standard Ollama image, sets `OLLAMA_VULKAN=1`, and requires `ollama ps` to prove actual GPU execution. If proof fails, Auto rebudgets to CPU and forced Vulkan fails closed.
...
If an older v14.5.44 checkout fails after DirectML preflight with `Argument types do not match`, upgrade the repository/full installer to the current v14.6.1 release. v14.5.45 introduced the underlying PowerShell compatibility fix. Do not require users to hand-edit PowerShell source or downgrade PowerShell. v14.5.45 removes the unsafe generic `New-Object` collection pattern repository-wide.

## v14.5.44 DirectML preflight support note

If the installer previously claimed `/dev/dxg` was missing even though `ls -l /dev/dxg` succeeded inside the selected WSL2 distro, use the full **v14.6.1** installer. v14.5.44 introduced the dedicated DirectML preflight fix inherited by current releases. DirectML now probes that path directly, retries as root, and reports D3D12/DXCore bridge libraries separately. A probe failure is no longer labeled as confirmed DXG absence. If an installer-owned DirectML venv already exists, repair also performs a real tensor probe; a failed environment remains Ollama-fallback capable and repair retries the DirectML environment.

## v14.5.43 universal repair-migration support note

For an older recognized LatticeVale installation, run the **full v14.6.1 installer** and choose **Resume / repair**. Do not install intermediate historical releases. The installer audits the existing managed stack, recovers/reads installer options, identifies its historical version/schema when available, creates a verified rollback backup, refreshes current installer-owned software/pins, replays required migrations, and regenerates canonical policy v13 from current hardware/backend/resource evidence.
...
For an existing install, rerun the full v14.6.1 installer and choose **Resume / repair**. Current releases use canonical adaptive policy v13. It derives WSL host reserve, DirectML host reserve, container budget, CPU quotas, service memory, model/context/concurrency limits, and runtime tuning from the resources and enabled topology actually visible now; it does not select a policy from a known-PC RAM table. If Auto-selected CUDA/ROCm/Vulkan execution later proves to be CPU through `ollama ps`, LatticeVale records runtime health and safely regenerates the resource-relevant Ollama budget as CPU. A changed hardware/runtime fingerprint allows a bounded retry. If a GPU mode was explicitly forced and cannot prove execution, repair that runtime or choose Auto/CPU; LatticeVale intentionally refuses GPU-sized assumptions.
...
For resource-policy troubleshooting, inspect `~/hermes-stack/resource-policy-report.txt` first. It is secret-free and records the current policy version, RAM/CPU class, GPU/offload state, VRAM coordination, generated service ceilings, and hardware/policy fingerprints. `./manage.sh audit` recomputes those fingerprints and compares effective/live Docker limits; a mismatch should be repaired with the full v14.6.1 installer rather than hand-editing generated state.


## v14.5.4 DirectML / low-memory support note

DirectML is optional and experimental. Current v14.6.1 keeps fail-closed model admission, but capacity no longer depends on a single `torch_directml` memory API. The selected adapter is correlated to canonical Windows hardware evidence; high-confidence dedicated-memory sources are preferred, legacy 32-bit values can only fill gaps as lower bounds, and UMA/shared-memory adapters receive a WSL-RAM-bounded admission ceiling. The default DirectML percentage remains bounded and may be reduced when a resource-relevant managed-Ollama GPU path shares the same adapter/vendor envelope. If capacity cannot be bounded safely, DirectML refuses model placement and Ollama remains the text fallback; Honcho embeddings remain on Ollama. Rerun the full **v14.6.1** installer and choose Resume / repair to refresh the isolated DirectML environment, hardware/backend state, and policy v13. Existing installations are not switched to DirectML automatically.
...
If a recognized LatticeVale installation falls below the ordinary managed-repair host-partition free-space floor, rerun the full v14.6.1 installer and choose **Option 7 — Cleanup / reclaim disk space** (or Option 3 for read-only verification). Cleanup is user-selected and confirmation-gated. It never removes LatticeVale containers, networks, volumes, tagged images, configured models, persistent application/database/vault/workspace/credential state, unverified backups, or the WSL VHDX. Options 1/2/4/5/6 remain storage-gated until the normal free-space floor is restored.

## v14.5.1 adaptive CPU/RAM / OOM support note

Current v14.6.1 uses canonical adaptive resource policy v13. CPU ceilings come from processors actually visible inside WSL and are distributed from one conserved Docker envelope; effective Compose `cpus` is verified against Docker `HostConfig.NanoCpus`. Managed Ollama receives RAM/context-derived provisional model sizing before artifacts are measurable, then measured model/context-aware sizing after download. GPU-backed managed Ollama records eligible NVIDIA/AMD topology and offload evidence; CPU execution is budgeted from the same live WSL envelope rather than a fixed host tier. Effective Compose `mem_limit` is verified against `HostConfig.Memory` in the same reconciliation pass. Changes to WSL CPU/RAM, enabled topology, model evidence, GPU coordination, or resource-relevant backend state invalidate and regenerate adaptive policy.

Policy v13 does not promise that every selected component set can run inside every RAM/VRAM size. It is feature-aware: only enabled services contribute to the minimum, but if that minimum cannot fit inside the post-reserve container budget, or if DirectML's coordinated VRAM envelope falls below its safe admission floor, LatticeVale fails the adaptive plan with an actionable message instead of generating unsafe ceilings. Increase WSL-visible RAM, choose a smaller model, deselect optional components, use native-Windows Ollama where appropriate, or disable adaptive container ceilings.

If `./manage.sh audit`, `verify`, or `repair --plan` reports `runtimePolicy PARTIAL` because a selected running container has Docker `OOMKilled=true`, use the full v14.6.1 release and choose **Resume / repair installation**. Canonical adaptive policy v13 treats older/stale policy state or changed hardware/backend/options fingerprints as regeneration triggers and reconciles generated Compose CPU/RAM ceilings without deleting volumes, changing profiles/providers, rotating Matrix identity, or taking ownership of global WSL memory settings. Memory/CPU ceilings, host reserve, model admission, context, and concurrency derive from the resources and selected workload actually visible at repair time; no fixed WSL-RAM shape is assumed. User `compose.override.yaml` remains final/authoritative.
...
The v14.4.3 RAM-efficiency and uninstaller behavior remains inherited. Historically, v14.4.84 retained policy v4, v14.5.1 introduced policy v9, and v14.5.2 inherited it unchanged; v14.5.4 advanced the DirectML/low-memory branch to policy v10. Current v14.6.1 uses canonical adaptive policy v13; v14.5.46 historically retained policy v11. Its Hermes floor and CPU ceiling still adapt to persistent secondary Matrix gateways and high Kanban concurrency, while the resource fingerprint now also covers CPU/RAM profile and managed-Ollama GPU topology/coordination. Generated startup helpers probe current resources/topology at runtime. User `compose.override.yaml` remains authoritative, global WSL RAM/reclaim settings remain user-owned, and Docker-unavailable uninstall still fails closed when runtime may remain.
