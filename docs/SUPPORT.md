...
For an older recognized LatticeVale installation, run the **full v14.6.0 installer** and choose **Resume / repair**. Do not install intermediate historical releases. The installer audits the existing managed stack, recovers/reads installer options, identifies its historical version/schema when available, creates a verified rollback backup, refreshes current installer-owned software/pins, replays required migrations, and regenerates canonical policy v12 from current hardware/backend/resource evidence.
