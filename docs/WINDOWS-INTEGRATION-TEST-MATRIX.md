...
- Legacy targeted-termination regression: prove the old helper contains `wsl --terminate <distro>`, run v14.4.84 Resume / repair, verify the bounded host transport reset succeeds, then verify the rewritten shutdown helper contains no targeted termination and normal relay/firewall ownership remains correct.
