...
> **v14.4.5 repair/runtime-policy cases (inherited):** with adaptive limits enabled, verify a clean install writes policy v3; a repair from stale/missing policy v3 regenerates it even when `prepare_config` is already complete; changed overlays force Compose reconciliation before success; and `manage.sh restart` reconciles a freshly regenerated policy before restarting. v14.4.6 supersedes the temporary version-only managed-refresh trigger; current refresh behavior is tested in the v14.4.6 row above. `compose.override.yaml` must remain last. Inherited v14.4.4 metadata-race and v14.4.3 uninstall-preservation cases remain required.
