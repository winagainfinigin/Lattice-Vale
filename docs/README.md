# LatticeVale v14.5.2 — Stable

> **v14.5.2:** current direct-install release. It adds the isolated Option 7 cleanup/reclaim path and a low-space recovery gate that permits only Verify or Cleanup for a recognized managed stack below the ordinary repair floor. It otherwise inherits v14.5.1 resource policy v9, OOM-aware audit/repair detection, model-aware managed-Ollama sizing, and live Docker CPU/RAM convergence unchanged. Existing installations should run the full v14.5.2 release and choose Resume / repair unless they specifically need Option 7 storage cleanup.
...
**v14.4.5 introduced the current repair-convergence mechanics over v14.4.4.** It makes adaptive RAM policy v3 an explicit repair obligation instead of relying on a possibly completed `prepare_config` checkpoint, reconciles changed Compose resource policy into running containers, and prevents final success while runtime policy is stale. v14.4.85 retained those mechanics and the v14.4.83 migration of enabled policy-v3 state to policy v4; v14.5.1 advanced enabled adaptive state to policy v9; v14.5.2 retains that policy unchanged to correct audited Hermes cgroup starvation and make managed-Ollama sizing model/context-aware, while retaining v14.4.8's Hermes/web maintenance and v14.4.6's managed-refresh revision/age/explicit-force model.
