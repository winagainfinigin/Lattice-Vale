...
> **v14.4.84 Hotfix 2:** same-version public hotfix. Existing Hotfix 1 installs should run the Hotfix 2 full release and choose Resume / repair. It fixes both reconcile validation races found during pre-release testing: managed services such as Ollama can still be settling after Compose reports them running, and Hermes API/Dashboard readiness can be invalidated by the final default-gateway restart. Hotfix 2 waits for selected managed-service, Matrix/Synapse, API, and Dashboard readiness, then verifies gateway-owned surfaces again after the final gateway mutation in both reconcile and the later Kanban/gateway stage. Normal Start/Restart/Update use the same post-gateway readiness barrier. Internal reconcile/gateway checkpoints are revision 4; fresh installs use the corrected ordering directly.
