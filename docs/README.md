# LatticeVale v14.5.1 — Stable

> **v14.5.1:** current direct-install release. It retains the v14.5.0 read-only planner architecture and fixes adaptive memory policy v4 starvation found in a real full-stack audit. Policy v9 preserves the audited Hermes/Honcho balance and makes managed Ollama sizing model/context-aware, derives CPU ceilings from the processors actually visible to WSL, verifies live Docker CPU and RAM ceilings, and refuses selected-service combinations whose minimum safe memory set cannot fit the bounded aggregate WSL budget; running-container Docker OOM history remains a repairable audit condition. Existing installations should run the full v14.5.1 release and choose Resume / repair.
...
> **v14.4.84:** prior lifecycle-repair release. It removes targeted `wsl --terminate` from the Windows Shut Down shortcut and adds a Resume / repair migration that detects the exact legacy installer-owned helper, performs bounded `wsl --shutdown` + `WslService` transport recovery, verifies the same distro, and rewrites the shortcut. It retains v14.4.83 resource policy v4, Redis/Valkey overcommit, Ubuntu Pro removal, and canonical public launchers. Its WSL lifecycle protections are inherited by v14.4.85. See [`PATCH-NOTES.md`](PATCH-NOTES.md) and [`SUPPORT.md`](SUPPORT.md).

**Unofficial, inspectable WSL2-native self-hosted Hermes Agent platform with a Windows bootstrap/host-integration layer.**

LatticeVale deploys and repairs Hermes Agent inside an **existing supported Ubuntu WSL2 distro** using Docker Engine/Compose; core runtime and lifecycle operation remain in WSL. Optional integrations include the Hermes Dashboard, named profiles, Kanban orchestration, Matrix/Synapse, SearXNG, QMD/Obsidian, Honcho memory, managed or native-Windows Ollama, private Windows Tailscale access, adaptive container resource ceilings, Ubuntu/WSL maintenance policy, and Windows lifecycle helpers.
...
**v14.4.5 introduced the current repair-convergence mechanics over v14.4.4.** It makes adaptive RAM policy v3 an explicit repair obligation instead of relying on a possibly completed `prepare_config` checkpoint, reconciles changed Compose resource policy into running containers, and prevents final success while runtime policy is stale. v14.4.85 retained those mechanics and the v14.4.83 migration of enabled policy-v3 state to policy v4; v14.5.1 now advances enabled adaptive state to policy v9 to correct audited Hermes cgroup starvation and make managed-Ollama sizing model/context-aware, while retaining v14.4.8's Hermes/web maintenance and v14.4.6's managed-refresh revision/age/explicit-force model.
...
When adaptive container resource limits are enabled, policy v9:
...
**Update / repair installer-managed software** is the controlled on-demand updater for an existing managed stack. Choose it from the Windows installer's existing-stack menu when you want to apply the software versions/channels declared by the LatticeVale bundle you are currently running **now**, without waiting for the periodic repair window. It first requires a verified **bundle-owned pre-update safety backup** that is independent of the currently installed `manage.sh`, then forces the managed refresh and runs the normal repair/verifier sequence. The backup helper can read container-owned persistent files, validates its database dumps/archive, restores the previously-running containers, and fails before refresh with step-specific diagnostics if the backup is not trustworthy. This is the recommended way to use a newer LatticeVale release as an updater.
