...
- Adds durable GPU opt-out, three DirectML text-fallback modes, independent Ollama text/embedding roles, DirectML `torch.no_grad()`, strict no-fallback behavior, and conserved automatic CPU allocation.
- Fixes the fail-closed DirectML path calling an undefined `text_fallback_enabled` helper during infrastructure setup.
- Adds a narrowly scoped Transformers 4.46.3 Qwen2/Qwen2.5 DirectML mask compatibility shim that replaces known-problematic in-place boolean multiplication/`masked_fill` operations with equivalent `torch.where` construction inside the installer-owned DirectML process only.
- Improves DirectML self-test failure reporting by printing the bounded curl error and gateway-log tail before fallback/fail-closed handling, so a model-process crash is diagnosable instead of appearing as only “no HTTP response.”
- Strengthens policy-v13 host-RAM budgeting for DirectML: the WSL-host reserve scales smoothly at about 30% with a bounded 2 GiB floor when affordable and an 8 GiB cap, with no fixed RAM tier. The formula is monotonic at MiB granularity so increasing WSL RAM cannot reduce the calculated container budget at a rounding/tier boundary.
- Expands migration/release validation for schema 21→23 and 22→23, 3,328 resource-topology combinations, boundary/irregular RAM cases, fixture process isolation, and repository-patch GPU recovery completeness.
...
- Advances installer options to schema 23, resource policy to 13, and retains managed repair refresh revision 4. Generation and verification consume the same canonical resource-budget function and generated state self-validates before downstream mutation.
...
- Extends live adaptive-resource convergence from memory to CPU: effective Compose `cpus` is verified against Docker `HostConfig.NanoCpus`, so WSL processor-count changes cannot leave stale live quotas behind current-looking policy files.
- Removes the old proportional low-memory fallback. Policy v9 calculates minima from enabled services plus the configured Hermes profile/Kanban topology and refuses a selected-service plan that cannot meet those minima inside the managed budget, preventing superficially valid but operationally implausible tiny hard ceilings.
- Keeps the aggregate managed memory budget bounded and leaves global WSL `memory` / `autoMemoryReclaim` user-owned. <=6 GiB keeps 30%; CPU-backed managed Ollama on >6-12 GiB uses a 10% reserve to support a 4608 MiB viability floor; other >6-24 GiB shapes use 20%; >24 GiB uses 15%. Hermes keeps a 1024 MiB common-topology floor/weight 8 and Honcho API its 512 MiB preferred floor/weight 4. The Hermes floor covers the default gateway plus up to one secondary Matrix gateway and Kanban concurrency up to 3; extra persistent secondary Matrix gateways add 192 MiB each beyond the first and Kanban slots above 3 add 96 MiB each, capped at 4096 MiB.
- On the audited ~9946 MiB CPU-backed full-stack shape, the planner now generates approximately Hermes 1040 MiB, Honcho API 528 MiB, Honcho deriver 400 MiB, and Ollama 4640 MiB. This keeps the policy-v4 Hermes correction while adding roughly half a GiB of Ollama hard-limit headroom over the v6 4128 MiB ceiling.
- Existing adaptive policy-v8-or-older fingerprints are intentionally stale under the final v14.5.1 build and converge through the existing preservation-first Resume / repair, Start, Restart, and root startup-helper resource refresh paths; user `compose.override.yaml` remains last/authoritative.
- `state-audit.py` now treats Docker `OOMKilled=true` on a **currently running selected container** as a repairable runtime-policy degradation. This closes the false-healthy case where Hermes endpoints recovered between repeated child-process OOM kills. `./manage.sh verify` and `repair --plan` inherit that classification through the existing audit interface.
- Strengthens the same-version v14.5.1 repair path by persisting each generated per-service MiB ceiling and comparing it with Docker's live `HostConfig.Memory` during final reconciliation. A v14.5.0 Hermes container still running with the old 544 MiB cgroup now invalidates reconcile verification and is recreated/reconfigured through normal Compose reconciliation; clean installs prove the same live limits after first creation.
- Adds `v14.5.1-resource-policy-oom-fixtures.py` and updates cumulative resource-policy/version regression expectations without changing third-party pins, checkpoint revisions, Matrix identity behavior, WSL distro ownership, or the v14.5.0 read-only planner architecture.
- Adds `v14.5.1-option-topology-resource-fixtures.py` to cover public setup shapes: optional Matrix/search/QMD/Honcho combinations, managed CPU/GPU/native-Windows Ollama, 0-8 additional profiles, Kanban concurrency 1-8, adaptive ceilings disabled, and profile-heavy safe-refusal cases. Hermes CPU retains the ordinary ~75% ceiling for common topology and may rise toward 100% under additional persistent gateway/worker pressure.
- Fixes generated root/Windows-start helpers so WSL CPU/RAM, adaptive-limit selection, Matrix-profile gateway count, and Kanban concurrency are evaluated when the helper runs. Auto-start therefore adapts after later resource/profile changes instead of retaining bootstrap-time values.
- Same-version live-repair hardening: exact Hermes s6 gateway readiness now allows a bounded 60-second stable-start window, tolerates transient s6 status-probe failures as startup noise, and performs one exact-slot `s6-svc -U` recovery before declaring reconcile broken. This fixes a real repair where the former 20-second gate failed even though `./manage.sh verify 300` became healthy shortly afterward.
- Fixes the exact-gateway diagnostic logger so `/bin/sh` never interprets a `---`-prefixed `printf` format as an option, and makes the v14.5 read-only override note version-neutral. Adds `v14.5.1-delayed-gateway-reconcile-fixtures.py` for the observed slow-start path.
