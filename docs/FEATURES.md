...
- Three DirectML text-fallback policies: native Windows Ollama, managed WSL/Docker Ollama, or fail-closed none.
- Independent Ollama roles for normal text, DirectML text fallback, and Honcho embeddings.
...
> **v14.6.0 same-version GPU portability hardening:** DirectML capacity can come from runtime reporting or canonical selected-adapter Windows DXDiag/64-bit registry evidence; lower-bound legacy telemetry cannot cap stronger measurements. UMA/shared memory is bounded by live WSL RAM. DirectML fallback health is fingerprinted separately from resource topology, so safe Ollama fallback does not invalidate an otherwise correct policy. Generic `other` DirectX 12 adapters can reach DirectML runtime qualification instead of being rejected by vendor name alone.

> **v14.5.47 WSL GPU recovery:** DirectML exports the selected Windows adapter at the WSL D3D12 layer before importing `torch_directml`, records selected-adapter DXDiag dedicated VRAM as a guarded runtime fallback, fingerprints hard-fallback state so changed runtimes retry once, and exposes read-only `diagnose`/Windows GPU audit tooling. Managed Ollama adds a Vulkan option for WSL DRM-render devices and accepts it as GPU execution only after real `ollama ps` offload proof.
...
- reserves WSL/Docker/Windows-facing headroom adaptively from the live WSL RAM envelope
...
- derives Synapse cache and PostgreSQL `shared_buffers` tuning from live resources and generated service allocations while retaining Honcho PostgreSQL `max_connections=200`
- recalculates when relevant WSL-visible resources change or an older adaptive-policy revision must be migrated

Current policy v13 retains bounded host-headroom/OOM protections while replacing historical RAM-size tiers with continuous canonical calculations. Host reserve scales from the RAM WSL actually exposes, DirectML receives its own bounded WSL-host reserve when configured, and the remaining container budget is water-filled only across enabled services. Hermes topology, Kanban concurrency, actual model/context inputs, backend type, and measured GPU/UMA capability feed the same canonical planner. CPU quotas scale from visible CPUs and workload pressure; allocator/Synapse/PostgreSQL tuning derives from live resources and the generated service allocations rather than fixed small/large-host buckets. If selected services cannot meet safety floors inside the current budget, policy v13 refuses the plan instead of crushing every service proportionally. These are **ceilings, not reservations**, and are not a global WSL memory cap.

DirectML capacity is derived independently from Linux-native GPU inventory. The selected Windows adapter is correlated by stable identity; high-confidence dedicated/full-capacity evidence outranks legacy 32-bit lower bounds, and UMA/shared-memory admission is conservatively bounded by live WSL RAM. CUDA, ROCm, Vulkan, native-Windows Ollama, DirectML, and CPU are qualified independently. GPU memory is never blindly aggregated across devices unless the backend/resource path explicitly supports that topology. Runtime-health state has a separate fingerprint: transient DirectML failure can activate Ollama fallback without invalidating the stable DirectML resource topology, while resource-relevant managed-Ollama acceleration changes can trigger rebudgeting.


The revised v14.5.47 package makes host-memory budgeting canonical across generation and verification. The same calculation covers WSL RAM size, resolved managed-Ollama acceleration, whether managed Ollama is enabled, and whether DirectML is selected; verification checks total reserve, DirectML host reserve, and resulting container budget. DirectML budgeting is intentionally independent of Linux-native GPU enumeration because `/dev/dxg`/D3D12 is a separate WSL-host execution path.

Clean installs generate policy v13 before first container creation and verify the resulting live Docker CPU and memory ceilings. The resource fingerprint includes Matrix-enabled secondary gateway count, Kanban concurrency, the derived Hermes topology floor, RAM/CPU profile, selected managed-Ollama acceleration, GPU topology, per-GPU overhead, DirectML percentage, and shared-vendor state. The generated root/Windows-start helper evaluates current CPU, RAM, enabled-limit state, profile topology, Kanban concurrency, and acceleration state at helper runtime so auto-start paths do not retain an install-time resource snapshot. Resume / repair and managed start-time resource reconciliation migrate enabled policy-v2 through policy-v10 state. Final reconciliation compares effective merged Compose `mem_limit` and `cpus` with each selected container's live Docker `HostConfig.Memory` and `HostConfig.NanoCpus`. A running selected container with Docker `OOMKilled=true` is reported as a repairable runtime-policy degradation rather than accepted as healthy.
