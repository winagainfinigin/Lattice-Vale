# LatticeVale v14.5.46 — Complete Features and Install Options Reference

> **v14.5.46 GPU-aware onboarding:** when local AI/Honcho requires local inference, the questionnaire inventories recognized Windows GPUs plus selected-distro WSL capability state and marks a recommended backend. NVIDIA + verified WSL `nvidia-smi` recommends managed Ollama NVIDIA; AMD + `/dev/kfd` + `/dev/dri` recommends managed Ollama ROCm; otherwise a healthy DirectML bridge recommends DirectML for AMD/Intel/Qualcomm/other recognized DX12 hardware. Selected DirectML/NVIDIA WSL prerequisites are reused or provisioned idempotently. A recommendation never substitutes for runtime proof.

> **v14.5.45 PowerShell compatibility hotfix:** all LatticeVale PowerShell generic collections now use direct `.NET ::new()` construction instead of `New-Object`. Generic-list helper returns use explicit `.ToArray()` where appropriate, preventing the PowerShell 7.6 `Argument types do not match` binder failure without requiring users to patch scripts or downgrade PowerShell.

> **v14.5.44 DirectML preflight hotfix:** DirectML now checks `/dev/dxg` directly inside the selected WSL2 distro, retries the read-only check as root after a normal-user miss/failure, reports D3D12/DXCore bridge-library projection separately, and optionally runs a real tensor probe when an existing installer-owned DirectML environment is available. Probe failure is no longer treated as proof that `/dev/dxg` is missing.
...
> **v14.5.43 preservation rule:** historical/versionless option schemas are normalized to current schema 20 while application/user state is preserved. Legacy managed Ollama with no acceleration setting is adopted as explicit CPU for safe resource ownership, not silently switched to GPU.
...
> **v14.5.42 canonical-policy diagnostics:** one finalized resource object drives Compose/state/audit; separate hardware and policy SHA-256 fingerprints plus secret-free `resource-policy-report.txt` explain exactly why each CPU/RAM/VRAM/context limit was chosen. v14.5.42 used a six-shard 135-fixture deterministic suite; v14.5.43 extended that contract to 136 fixtures; v14.5.44 extended it to 137 fixtures; v14.5.45 extended it to 138 fixtures; current v14.5.46 extends it to 139 fixtures with the universal-repair migration gate.
...
> **v14.5.3 local-AI addition:** the questionnaire can explicitly select **Ollama** or the experimental **PyTorch DirectML gateway** for local text inference. DirectML routes Hermes + Honcho text through a WSL-host OpenAI-compatible endpoint; Ollama remains required for Honcho embeddings and fallback.
...
This file consolidates the **current, available LatticeVale v14.5.46 capabilities and installer choices** scattered across the release documentation. The source basis was the complete audited v14.3.43 runtime release tree promoted to v14.4.0 without runtime behavior changes: 61 Markdown/text documentation files (28 current/release documents and 33 explicitly archival v13 documents), plus the current installer/configuration source used to resolve historical or ambiguous documentation.

Historical v13 notes are treated as compatibility lineage only. A feature is described here as current only when it is retained by the v14.5.46 documentation/source. Superseded behavior is called out separately rather than presented as an available current option.
...
Current policy v11 retains the bounded host-headroom/OOM protections while preserving the policy-v9 corrections for policy-v4 Hermes starvation and policy-v6 CPU-backed Ollama `memory.max` pressure. It keeps the Hermes share topology-aware for public multi-profile/Kanban configurations and adds explicit compact/balanced/large WSL-RAM profiles plus compact/balanced/high CPU profiles. It reserves 30% of WSL-visible RAM on <=6 GiB; on >6-12 GiB it uses 10% when CPU-backed managed Ollama is selected and 20% otherwise; >12-24 GiB uses 20%; and >24 GiB uses 15%, with bounded minimum/maximum reserve values. Hermes keeps a 1024 MiB common-topology minimum and Honcho API 512 MiB on the normal profile; CPU-backed managed Ollama uses a 4096 MiB provisional floor before model artifacts are measurable, then a model/context-aware floor after download. The 1024 MiB Hermes baseline covers the default gateway plus up to one secondary Matrix gateway and Kanban concurrency up to 3; each additional persistent secondary Matrix gateway adds 192 MiB and each Kanban slot above 3 adds 96 MiB, capped at 4096 MiB. Hermes CPU normally uses about 75% of WSL-visible CPUs but gains bounded ceiling headroom toward 100% as topology pressure increases. GPU-backed Ollama uses the medium CPU ceiling because inference is expected to be offloaded; CPU-backed Ollama retains the heavy CPU ceiling. The minimum calculation is service-aware: lighter selections can fit smaller WSL allocations, but if the selected set cannot meet its defined hard minima, policy v11 refuses to generate the overlay instead of proportionally crushing every service. These are **ceilings, not reservations**, and are not a global WSL memory cap.

Policy v11 inherits the policy-v10 DirectML low-memory behavior: DirectML host memory is a floor on the existing non-container reserve rather than an additional reserve, <=12 GiB WSL allocations use tighter supporting-service minima/useful ceilings while preserving Hermes at 1024 MiB, and DirectML-mode managed Ollama uses 4096 context with zero keep-alive as transient embedding/fallback infrastructure. It additionally inventories eligible NVIDIA/AMD GPUs, records count/minimum/maximum/aggregate VRAM, and coordinates same-vendor DirectML/Ollama VRAM. Similar-size GPU sets receive approximately half-and-half envelopes; heterogeneous sets derive Ollama's fixed per-GPU reserve from the smallest adapter and reduce DirectML's percentage against the largest adapter. `OLLAMA_GPU_OVERHEAD` is written as a per-GPU byte reserve. Managed GPU context is the smaller of the RAM recommendation and usable-VRAM recommendation, capped at 64k for coexistence. After model load, bounded `ollama ps` verification must prove GPU offload: Auto falls back and re-budgets as CPU if Ollama explicitly reports CPU execution, while a forced NVIDIA/AMD selection fails closed.

Clean installs generate policy v11 before first container creation and verify the resulting live Docker CPU and memory ceilings. The resource fingerprint includes Matrix-enabled secondary gateway count, Kanban concurrency, the derived Hermes topology floor, RAM/CPU profile, selected managed-Ollama acceleration, GPU topology, per-GPU overhead, DirectML percentage, and shared-vendor state. The generated root/Windows-start helper evaluates current CPU, RAM, enabled-limit state, profile topology, Kanban concurrency, and acceleration state at helper runtime so auto-start paths do not retain an install-time resource snapshot. Resume / repair and managed start-time resource reconciliation migrate enabled policy-v2 through policy-v10 state. Final reconciliation compares effective merged Compose `mem_limit` and `cpus` with each selected container's live Docker `HostConfig.Memory` and `HostConfig.NanoCpus`. A running selected container with Docker `OOMKilled=true` is reported as a repairable runtime-policy degradation rather than accepted as healthy.
...
# 8. Current managed software/source pins documented by v14.5.46
