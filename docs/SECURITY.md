...
The shortcut invokes Windows PowerShell with `-ExecutionPolicy Bypass` **for that child process only**. Start/Shut Down bind directly to the selected distro/Linux user/stack and invoke `./manage.sh start|stop` through WSL `--cd`; they do not depend on a nested login-shell quoting wrapper. Start therefore follows the saved install options. v14.4.85 retains the v14.4.84 Shut Down behavior: it checks that the target distro is running and requests `manage.sh stop`, but deliberately does **not** use targeted `wsl.exe --terminate` or global `wsl --shutdown`. Existing-install repair recognizes the exact older LatticeVale-owned helper that used targeted termination, performs a bounded WSL/WslService transport reset, and then replaces the helper.
