> **v14.5.0 current release:** `VERSION.txt` must be 14.5.0. Run `LatticeVale-Core/tests/v14.5.0-readonly-planner-fixtures.py` and `v14.5.0-release-identity-fixtures.py` plus the inherited v14.4.85 reconcile/readiness/update-backup fixtures and the full deterministic suite. Verify the new planner/config reader is read-only, `./manage.sh plan` and `./manage.sh repair --plan` do not mutate managed or user-owned configuration, `compose.override.yaml` remains opaque/user-owned, `checkpoint-metadata.json` exposes rather than replaces the working checkpoint revisions, and `./manage.sh audit-free` can identify a free/local default path without promising external free tiers.
