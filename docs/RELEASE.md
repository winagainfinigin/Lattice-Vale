> **v14.6.1 current release:** `VERSION.txt` must be 14.6.1. The release must pass architecture static checks, all deterministic fixtures, resume simulations, source-manifest exact coverage, repository-only release-content filtering, contamination checks, and ZIP member/byte verification. Canonical schemas are installer-options 23, runtime-policy 13, and managed-refresh revision 4.

> **Current 14.6.1 hotfix gate:** canonical installer-options schema is **23** and runtime-policy schema is **13**. The final gate must include schema-21→23 and schema-22→23 migration, durable GPU opt-out, all three DirectML text-fallback choices, role-separated Ollama behavior, the 3,328-case conserved-CPU property sweep, DirectML `no_grad`, fail-closed marker behavior, all six regression shards, both resume simulations, static/security/contamination checks, source-manifest verification, and validation of the freshly built ZIP contents.
...
> **v14.6.1 Matrix/Tailscale gate:** a requested Matrix Serve mapping is publishable only when `public_baseurl` reconciliation succeeds, `/_matrix/client/versions` is reachable, `/.well-known/matrix/client` returns the exact expected `m.homeserver.base_url`, and `/_matrix/client/v3/login` exposes a flow. Authentication backend migration remains out of scope for this hotfix.
...
> **GitHub packaging gate:** publishable archives must contain exactly one top-level `Lattice-Vale/` directory, no path traversal/reparse/symlink surprises, no generated bytecode/cache/private state, and no individual file over GitHub's 100 MiB regular-repository object limit. Community-facing README, LICENSE, CONTRIBUTING, SECURITY, SUPPORT, issue/PR metadata, and workflow files must describe the same v14.6.1 behavior.

> **v14.6.1 current release:** `VERSION.txt` must be 14.6.1. Run the canonical-architecture and release-policy fixtures, all inherited GPU/repair/preservation fixtures, static audit, both resume simulations, and the full 143-fixture deterministic contract. Hardware/backend/resource decisions must remain canonical and adaptive across qualified CPU-only, AMD, NVIDIA, Intel, DirectML, CUDA, ROCm, Vulkan, native-Windows, single-/multi-GPU, and heterogeneous systems; no production policy may target a fixed RAM or machine topology.
> **Mandatory v14.6.1 release gate:** all six shards must total exactly 144 PASS / 0 FAIL / 0 SKIP. Both resume simulations must pass. Corrected v14.5.47 durable/schema-21 state must migrate preservation-first through schema 23 and regenerate derived architecture state. The exact v14.6.0 parent plus the repository patch set must reproduce the qualified v14.6.1 repository tree byte-for-byte, and a fresh full release ZIP must match the declarative release-content policy and source manifest.
...
Test a managed v14.4.4-style install whose adaptive resource fingerprint/overlay is missing or policy-v2 while `prepare_config` is already checkpointed complete. Under the current release, Resume / repair must regenerate policy v13, retain `compose.override.yaml` as the final layer, mark runtime reconciliation pending, and apply the changed Compose configuration to running containers before final success. A post-run `./manage.sh audit` must report `runtimePolicy CONFIGURED`.
