> **v14.4.84 Hotfix 2:** keep `VERSION.txt` at 14.4.84. Run both `LatticeVale-Core/tests/v14.4.84-hotfix2-reconcile-startup-fixtures.py` and `LatticeVale-Core/tests/v14.4.84-hotfix2-post-gateway-readiness-fixtures.py`, plus Hotfix 1 and WSL shortcut fixtures. Verify existing Hotfix 1 checkpoint state is replayed with `reconcile` and `kanban_gateway` revision 4; fresh install and repair wait for selected managed Ollama, Matrix/Synapse, API, and Dashboard readiness; API/Dashboard readiness occurs after the final default-gateway lifecycle mutation in both lifecycle stages; the later Kanban gateway reload cannot recreate the race; and normal Start/Restart/Update also wait for gateway-owned surfaces before returning success.
