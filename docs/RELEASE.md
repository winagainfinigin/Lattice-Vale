> **v14.4.85 current release:** `VERSION.txt` must be 14.4.85. Run `LatticeVale-Core/tests/v14.4.85-reconcile-startup-fixtures.py`, `v14.4.85-post-gateway-readiness-fixtures.py`, and `v14.4.85-update-backup-audit-fixtures.py`, plus the inherited Hotfix 1 and v14.4.84 WSL shortcut fixtures. Verify existing v14.4.84 checkpoint state is replayed with `reconcile` and `kanban_gateway` revision 4; fresh install and repair wait for selected managed Ollama, Matrix/Synapse, API, and Dashboard readiness after final gateway lifecycle mutations; Option 6 completes a bundle-owned verified safety backup before managed refresh; and Option 3 prints a fresh read-only Linux/Windows audit.
