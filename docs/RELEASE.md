> **v14.5.46 current release:** `VERSION.txt` must be 14.5.46. Run `LatticeVale-Core/tests/v14.5.46-gpu-onboarding-fixtures.py`, retained v14.5.45 PowerShell compatibility, v14.5.44 DirectML preflight, v14.5.43 universal-repair, and v14.5.42 hardware-policy fixtures, static audit, and the full 139-fixture deterministic suite. GPU recommendations must use Windows inventory plus direct WSL capability state, and selected-path prerequisite acquisition must remain idempotent/fail-closed.
> **Mandatory v14.5.46 release gate:** all six shards must total exactly 139 PASS / 0 FAIL / 0 SKIP, with the release tail including the v14.5.46 GPU-onboarding fixture. The exact v14.5.45 parent must reproduce the qualified v14.5.46 tree through its Git patch, and a freshly extracted release ZIP plus source manifest must match the qualified tree byte-for-byte.
...
Test a managed v14.4.4-style install whose adaptive resource fingerprint/overlay is missing or policy-v2 while `prepare_config` is already checkpointed complete. Under the current release, Resume / repair must regenerate policy v11, retain `compose.override.yaml` as the final layer, mark runtime reconciliation pending, and apply the changed Compose configuration to running containers before final success. A post-run `./manage.sh audit` must report `runtimePolicy CONFIGURED`.
...
Verify the inherited v14.4.3 behavior remains intact under current policy v11: a clean adaptive install generates resource policy v11, an existing policy-v2 through policy-v10 adaptive install regenerates to v11 on repair/start, the WSL-visible resource fingerprint is persisted, `compose.override.yaml` remains last, and Honcho PostgreSQL retains `max_connections=200`. Verify LatticeVale does not write global WSL `memory` or `autoMemoryReclaim` settings for this feature.
