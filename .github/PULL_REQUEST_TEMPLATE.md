> **v14.5.1 quality checklist:** read-only planner/config-state code must never write installer/user state; `compose.override.yaml` remains opaque/user-owned; applying repair remains the existing installer path; checkpoint revisions must stay shared with `checkpoint-metadata.json` plus fallback; and all inherited v14.4.85 preservation/readiness guarantees remain required.
