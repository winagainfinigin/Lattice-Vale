> **v14.4.84 quality checklist:** preserve all v14.4.3 RAM/uninstaller and earlier ownership boundaries; live metadata repair may ignore only entries that actually vanished; resource audit must use WSL/process-visible CPU count; and a version-only bundle bump must not force managed image/source rebuilds unless the managed-refresh revision/age/legacy-state gate or explicit Option 6 requires it. Direct public 14.4.2→14.4.84 repair must still converge through refresh revision 1→2 and resource policy v2/v3→v4. Hermes clean/repair may fill only missing LatticeVale-managed web/browser defaults; explicit browser/provider/gateway/environment/timeout choices and user-owned `SOUL.md`/model policy must remain untouched.
...
- [ ] Windows lifecycle shortcuts contain no targeted `wsl --terminate`; v14.4.84 repair detects legacy owned shortcut helpers, resets WSL/WslService transport, and rewrites them.
